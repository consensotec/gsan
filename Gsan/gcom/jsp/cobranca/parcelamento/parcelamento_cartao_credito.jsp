...
function validarCartaoCredito() {
	...
	if (!form.parcelamentoRecebidoParametro.value && !form.idParcelamento){
...
function adicionarTransacaoCartaoDebito(){
	if (validarCartaoDebito()){
		...
		abrirPopupComSubmit('exibirAdicionarPagamentoCartaoCredito.do?inicio=sim&valorTotal=' + valorTotalSelecionado,450,532,'Adicionar');
	}
}
...
function validarRadio() {
	...
	if (RadioNaoVazioMensagemPorNome("Parcelamento", "idParcelamento")) {
		form.submit();
	}
}
...
<html:form action="/exibirParcelamentoCartaoCreditoConfirmarAction.do"
	...
	<input type="hidden" name="parcelamentoRecebidoParametro" value="${sessionScope['parcelamentoRecebidoParametro']}">
		...
		<tr>
			...
			<td width="625" valign="top" class="centercoltext">
				...
				<tr>
					...
					<td colspan="2">
						<html:radio property="modalidadeCartao" value="<%=""+ConstantesSistema.MODALIDADE_CARTAO_CREDITO%>"
							onclick="modificarModalidade();" disabled="${sessionScope['parcelamentoRecebidoParametro']}"/>
						<strong>Crédito</strong>
						<html:radio property="modalidadeCartao" value="<%=""+ConstantesSistema.MODALIDADE_CARTAO_DEBITO%>"
							onclick="modificarModalidade();" disabled="${sessionScope['parcelamentoRecebidoParametro']}"/>
										...
										<tr>
											<td>
												...
												+ Util.formatarMoedaReal(new BigDecimal(conta.getConta().getValorTotalConta())).trim()%>
													...
													+ " Juros de Mora: "
													+ Util.formatarMoedaReal(conta.getValorJurosMora()).trim()
													+ " Atualização Monetária: "
													+ Util.formatarMoedaReal(conta.getValorAtualizacaoMonetaria()).trim()%>">
				...
				<tr>
					...
					<table width="100%" border="0">
						<tr>
							...
							<logic:present name="colecaoTransacao" scope="session">
								<%
									if (((Collection) session.getAttribute("colecaoTransacao")).size() == 1) {
								%>									
									<td align="right"><input type="button" class="bottonRightCol"
										value="Adicionar" tabindex="11" style="width: 80px" id="adicionar"
										onclick="adicionarTransacaoCartaoDebito();" disabled="disabled">
									</td>
								<%
									} else {
								%>
							...
							<logic:notPresent name="colecaoTransacao" scope="session">
								<td align="right"><input type="button" class="bottonRightCol"
									value="Adicionar" tabindex="11" style="width: 80px"
									onclick="adicionarTransacaoCartaoDebito();">
								</td>
