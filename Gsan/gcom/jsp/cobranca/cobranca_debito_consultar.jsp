...
function validaEnterClienteSuperior(tecla, caminhoActionReload, nomeCampo) {
	...
	if(form.codigoClienteSuperior.value.length > 0) {
		...
		form.idClienteTipo.disabled = true;
		form.idEsferaPoder.disabled = true;
		form.buttonGerarRelatorio.disabled = true;
		...
		form.tipoRelacao.disabled = false;
		form.idClienteTipo.disabled = false;
		form.idEsferaPoder.disabled = false;
		form.buttonGerarRelatorio.disabled = false;
...
function habilitarDesabilitarAoInformarClienteTipoOuEsferaPoder(){
	var form = document.forms[0];

	if(form.idClienteTipo.value!="-1" || form.idEsferaPoder.value!="-1")
	{
		form.codigoImovel.disabled = true;
		form.codigoClienteSuperior.disabled = true;
		form.codigoCliente.disabled = true;
		form.buttonConsultar.disabled = true;
	}
	else
	{
		form.codigoImovel.disabled = false;
		form.codigoClienteSuperior.disabled = false;
		form.codigoCliente.disabled = false;
		form.buttonConsultar.disabled = false;
	}
}
</script>
</head>
		...
		<tr>
			...
			<table width="100%" border="0">
			  ...
			  <tr> 
				...
				<logic:present name="permissaoGeracaoRelatorioTippoClienteEsferaPoder" scope="session">
				
					<tr>
						<td><strong>Tipo de Cliente:</strong></td>
						<td>
							<html:select property="idClienteTipo" onclick="habilitarDesabilitarAoInformarClienteTipoOuEsferaPoder();">
							<html:option value="<%=""+ConstantesSistema.NUMERO_NAO_INFORMADO%>">&nbsp;</html:option>
								<logic:notEmpty name="colecaoClienteTipo">
					...
					<tr>
						...
						<td>
							<html:select property="idEsferaPoder" onclick="habilitarDesabilitarAoInformarClienteTipoOuEsferaPoder();">
							<html:option value="<%=""+ConstantesSistema.NUMERO_NAO_INFORMADO%>">&nbsp;</html:option>
								<logic:notEmpty name="colecaoEsferadoPoder">
								<html:options collection="colecaoEsferadoPoder"
									labelProperty="descricao" property="id" />
								</logic:notEmpty>
							</html:select>
						</td>
				...
				<logic:notPresent name="permissaoGeracaoRelatorioTippoClienteEsferaPoder" scope="session">
					<div style="visibility:hidden">
						<input name="idClienteTipo" />
						<input name="idEsferaPoder" />
					</div>
				</logic:notPresent>
				...
				<logic:present name="permissaoGeracaoRelatorioTippoClienteEsferaPoder" scope="session">
					...
					<tr>
						<td><strong>Selecione o Tipo de Relatório:</strong></td>
						<td>
							<html:select property="tipoRelatorio">
								<html:option value="1">Relatório Débitos Cliente</html:option>
								<html:option value="2">Relatório Débitos Resumido</html:option>
								<html:option value="3">Relatório Debitos Resumido por Responsável</html:option>
							</html:select>
						</td>
					</tr>
					...
					<tr>
						<td><strong>Formato do Relatório:</strong></td>
						<td>
							<label>
								<strong>PDF</strong>
								<html:radio property="formatoRelatorio" value="1"></html:radio>
							</label>
							
							<label>
								<strong>CSV</strong>
								<html:radio property="formatoRelatorio" value="2"></html:radio>
							</label>
				...
				<logic:notPresent name="permissaoGeracaoRelatorioTippoClienteEsferaPoder" scope="session">
					<div style="visibility:hidden">
						<input name="tipoRelatorio" />
						<input name="formatoRelatorio" />
					</div>
				</logic:notPresent>
				...
				<tr>
					...
					<td align="right">
						...
						<logic:present name="permissaoGeracaoRelatorioTippoClienteEsferaPoder" scope="session">
							<input name="buttonGerarRelatorio" type="button" class="bottonRightCol" value="Gerar Relatório">
						</logic:present>
						<logic:notPresent name="permissaoGeracaoRelatorioTippoClienteEsferaPoder" scope="session">
							<div style="visibility:hidden">
								<input name="buttonGerarRelatorio" />
							</div>
						</logic:notPresent>
								
						<gsan:controleAcessoBotao name="buttonConsultar" value="Consultar" onclick="javascript:validarForm(document.forms[0]);" url="consultarDebitoAction.do"/>				
