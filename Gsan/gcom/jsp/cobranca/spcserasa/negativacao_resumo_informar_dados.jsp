...
function adicionarGrupoCobrancaHidden(){
	var form = document.forms[0];
	var cobrancaGrupoSelecionadas = "";
	
	if(form.arrayCobrancaGrupo.disabled == false && form.arrayCobrancaGrupo.value != ""){
		
		for(var posicao = 0; posicao < form.arrayCobrancaGrupo.options.length; posicao++){
			
			if(form.arrayCobrancaGrupo.options[posicao].selected){
				
				if(cobrancaGrupoSelecionadas != ""){
					cobrancaGrupoSelecionadas += ",";
				}				
				cobrancaGrupoSelecionadas += form.arrayCobrancaGrupo.options[posicao].value;
			}
		}
	}
	return cobrancaGrupoSelecionadas;	
}
...
function recuperarDadosPopup(codigoRegistro, descricaoRegistro, tipoConsulta) {
    ...
    if (tipoConsulta == 'comandoNegativacao') {
		form.idNegativacaoComando.value = codigoRegistro;
   	    ...
   	    form.tituloComando.style.color = "#000000";
    }
}
...
function validaEnterEloPolo(tecla, caminhoActionReload, nomeCampo) {
	...
	if ('${sessionScope.gerarRelatorio}' == 'exibirResumoNegativacaoParametros') {
		caminhoActionReload ='exibirInformarDadosConsultaNegativacaoAction.do?menu=sim';
	} else {
		caminhoActionReload ='exibirInformarDadosConsultaNegativacaoAction.do?menu=sim&gerarRelatorio='+'${sessionScope.gerarRelatorio}';
	}	
...
function verificaSetorComercial(){
	...
	function submeter(){
		...
		if ('${sessionScope.gerarRelatorio}' == 'exibirResumoNegativacaoParametros') {
			form.action ='exibirInformarDadosConsultaNegativacaoAction.do?menu=sim';
		} else {
			form.action ='exibirInformarDadosConsultaNegativacaoAction.do?menu=sim&gerarRelatorio='+'${sessionScope.gerarRelatorio}';
		}	  	
	  	form.submit();
    }
...
function limparFormulario() {
	if ('${sessionScope.gerarRelatorio}' == 'exibirResumoNegativacaoParametros') {
		window.location.href='/gsan/exibirInformarDadosConsultaNegativacaoAction.do?menu=sim';
	} else {
...
<html:hidden property="descricaoQuadra" />
		...
		<td width="655" valign="top" class="centercoltext">
            ...
            <p>&nbsp;</p>
              <table width="100%" border="0" >
                ...
                <logic:notPresent name="relatorioNegativacoesExcluidas" scope="session">
	                <tr>
