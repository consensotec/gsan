...
function validarClientes(form){
	form.substituirClienteConta.value = true;
	if(document.getElementById("PERGUNTAR_SUBSTITUICAO_CLIENTE").value == "true"){
		var x;
		var sim=confirm("Caso deseje substituir o(s) cliente(s) vinculado(s) à conta pelo(s) cliente(s) atualmente vinculado(s) ao imóvel, selecione OK. Caso contrário selecione Cancelar.");
		if (sim==true){
			form.substituirClienteConta.value = true;
		}
		else{
			form.substituirClienteConta.value = false;
		}
	}
}
...
<INPUT TYPE="hidden" ID="PERGUNTAR_SUBSTITUICAO_CLIENTE" value="${requestScope.perguntarSubstituicaoCliente}"/>
...
<input type="hidden" name="substituirClienteConta" />
