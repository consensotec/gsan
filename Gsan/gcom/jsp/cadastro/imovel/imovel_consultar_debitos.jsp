...
function gerarExtratoDebitoCliente() {
	var form = document.forms[0];
	var mensagem = "Nenhum Documento foi selecionado para emitir extrado de débito do cliente.";		

	if (form.matriculaImovelDebitos.value != null && form.matriculaImovelDebitos.value != "") {
		if (CheckboxNaoVazioMensagemGenerico(mensagem, "")){
			form.contasGuiasSelecionadas.value = obterValorCheckboxMarcado();
			form.action = 'gerarRelatorioExtratoDebitoAction.do?consultarDebitoImovel=1&emitirExtratoDebitoCliente=1';
			form.submit();
		}
	} else {
		alert("Pesquise o imóvel");
...
<html:form action="/exibirConsultarImovelAction.do"
	...
	<input type="hidden" name="contasGuiasSelecionadas" />
...
<tr>
			...
			%>
				...
				<!-- GRID Historico de Clientes com Debito  -->
						...
						<table width="100%">
							<tr>  
								<logic:empty name="colecaoContaValores">
									<td align="left">
										<input type="button" name="" value="Emitir Extrato de Débito do Cliente" class="bottonRightCol" disabled="disabled"/>
									</td>
									<td align="right">
								...
								<logic:notEmpty name="colecaoContaValores">
									<logic:equal name="ConsultarImovelActionForm" property="indicadorEmissaoExtratoNaConsulta" value="<%=ConstantesSistema.SIM.toString()%>" >
										<td align="left">
											<input type="button" name="" value="Emitir Extrato de Débito do Cliente" class="bottonRightCol" onclick="gerarExtratoDebitoCliente()" >
										</td>
										<td align="right">
											<input type="button" name="" value="Emitir Extrato de Débito" class="bottonRightCol" onclick="gerarExtratoDebito()" >
										</td>
									</logic:equal>
