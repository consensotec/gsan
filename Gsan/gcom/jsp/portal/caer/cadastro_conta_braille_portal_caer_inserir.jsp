...
<html:html>
	<head>
		...
		<logic:present name="contaBrailleSolicitadaComSucesso" scope="request">
			<script type="text/javascript">
				$(document).ready(function(){
					$.blockUI({
						message : $('#mensagemBrailleSolicitadaComSucesso'),
						theme : true,
						title : 'Aviso'
					});
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				$('[name=dataExpedicao]').datepicker({
					showOn : 'both',
					buttonImage : '/gsan/imagens/portal/caer/icons/data.gif',
					maxDate : '+0d',
			...
			function validarRequerido(form){
				...
				if (form.telefoneContato.value.length < 2){
			...
			function verificarDadosCliente(){
				...
				form.action = "/gsan/exibirInserirCadastroContaBraillePortalCaerAction.do?ok=sim";
				form.submit();
	...
	<body>
		<div id="container">
	        ...
	        <div id="content">
				...
				<div id="fatura-email" class="serv-int">
	            	<h3>Solicitar conta em braille<span>&nbsp;</span></h3>
                	<p>CPF/CNPJ do Cliente: <em id="cpfFormatado"><bean:write name="cpfCnpj" scope="session" /></em></p>
	            	<html:form action="/inserirCadastroContaBraillePortalCaerAction.do?lojaVirtual=S"
						name="InserirCadastroContaBraillePortalCaerActionForm"
						...
						<html:hidden property="nomeCliente" value="${ExibirServicosPortalCaerActionForm.nomeUsuario}" />
						<html:hidden property="cpfCnpjCliente" value="${ExibirServicosPortalCaerActionForm.cpfCnpjSolicitante}" />
						
						<html:hidden property="confirmarNomeCliente" value="${ExibirServicosPortalCaerActionForm.nomeUsuario}" />
						<html:hidden property="confirmarCpfCnpjCliente" value="${ExibirServicosPortalCaerActionForm.cpfCnpjSolicitante}" />
						<html:hidden property="desabilitaCampos" />
						
						<fieldset>
							<legend>Solicitar Conta em Braille</legend>
								...
								<tr>
									<td style="text-align: left">
			                        	<span class="cmp-text-6">
											...
											<html:text property="email" size="40" maxlength="40" tabindex="1" style="text-transform: none;" />
										</span>										
										<span class="cmp-text-2">
			                            	<label for="telefoneContato" style="width: 170px;">Telefone para Contato<font color="#F00">*</font>:</label>
											<html:text property="telefoneContato" size="15" maxlength="14" tabindex="2" onkeypress="return isCampoNumerico(event);" 
											onkeyup="mascaraTelefone(this);" value ="(" alt="phone" />
										</span>
									</td>
								</tr>
								<tr>
									<td style="text-align: left">
										<span class="cmp-text-6">
											...
											<html:text property="nomeSolicitante" size="50" maxlength="50" tabindex="3" />
										</span>
										<span class="cmp-text-2">
			                            	<label for="cpf">CPF do Solicitante<font color="#F00">*</font>:</label>
			                            	<logic:present name="cpfCnpj" scope="session">
												<input type="text" name="cpfSolicitante" size="15" maxlength="14" tabindex="4" value='<bean:write name="cpfCnpj" scope="session" />' onkeypress="return isCampoNumerico(event);"/>
											</logic:present>
											<logic:notPresent name="cpfCnpj" scope="session">
												<input type="text" name="cpfSolicitante" size="15" maxlength="14" tabindex="4" value="" onkeypress="return isCampoNumerico(event);"/>
											</logic:notPresent>
										</span>
									</td>
								</tr>
								<tr>
									<td style="text-align: left">
										...
										<span class="cmp-text-5" style="width: 130px">
											...
											<html:text property="dataExpedicao" size="10" maxlength="10" style="margin-top: 5px;" tabindex="8"  
												onkeyup="javascript:mascaraData(this, event);" onkeypress="return isCampoNumerico(event);"/>
										</span>
									</td>
								</tr>
		...
		<logic:present name="contaBrailleSolicitadaComSucesso" scope="request">
			<div id="mensagemBrailleSolicitadaComSucesso" style="display:none; cursor: default;">
				<h3	style="text-align: justify; padding-top: 10px; padding-bottom: 10px;">
					<bean:write name="mensagemBrailleSolicitadaComSucesso" scope="request" />
				</h3>
	...
	<logic:present name="contaBraileJaSolicitada" scope="request">
		<div id="contaBraileJaSolicitada" style="display:none;cursor:default;">
	        ...
	        <h3 style="text-align:justify; padding-top:10px; padding-bottom: 10px;">Já existe uma solicitação de conta em braille para este imóvel</h3>
	        <p>
