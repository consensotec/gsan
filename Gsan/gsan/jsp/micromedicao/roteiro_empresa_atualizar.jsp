...
<script language="JavaScript">
	...
	function validarForm(form){
		if (validateInserirRoteiroEmpresaActionForm(form)){
			var mensagem = 'Confirma atualização de Roteiro Empresa?';
			if (confirm(mensagem)) {
	...
	function limparLeiturista(form) {
 	...
 	function recuperarDadosPopup(codigoRegistro, descricaoRegistro, tipoConsulta) {
    	...
    	if (tipoConsulta == 'localidade') {
			...
			form.action = 'exibirAtualizarRoteiroEmpresaAction.do?reloadPage=false';
			submeterFormPadrao(form);       	
    	} else 
	...
	function carregarQuadras(){
	    var form = document.forms[0];
		form.action = 'exibirAtualizarRoteiroEmpresaAction.do?carregarQuadras=Sim';
	...
	function removerSetorSelecionado(){
		...
		if (form.existeQuadrasCarregadas.value == 'S'){
			form.action = 'exibirAtualizarRoteiroEmpresaAction.do?removerSetor=Sim';				
			submeterFormPadrao(form);		
		}
	}
  ...
  function facilitador(objeto){
	...
	function voltar(){
	    var form = document.forms[0];
	  	form.action = 'exibirAtualizarRoteiroEmpresaAction.do?voltar=sim';
	    form.submit();		
	}	
</script>
</head>
...
<html:form action="/atualizarRoteiroEmpresaAction" name="InserirRoteiroEmpresaActionForm"
	type="gsan.gui.micromedicao.InserirRoteiroEmpresaActionForm" method="post"
		...
		<tr>
			...
			<table width="100%" border="0" align="center" cellpadding="0"
				...
				<tr>
					...
					<td class="parabg">Atualizar Roteiro Empresa</td>
...
<p>&nbsp;</p>
   			<table width="100%" border="0">
				...
				<tr>
					<td>Para atualizar o roteiro empresa, informe os dados abaixo:</td>
			 ...
			 <tr> 
				...
				<tr>
					...
					<td colspan="2"><html:text maxlength="3" tabindex="3" property="idLocalidade"
						size="3"
						onkeypress="javascript:validaEnterComMensagem(event, 'exibirAtualizarRoteiroEmpresaAction.do', 'idLocalidade', 'Localidade');" />
				...
				<tr>
					<td width="130"><strong>Leiturista:<font color="#FF0000">*</font></strong></td>
					<td colspan="2"><html:text maxlength="9" property="idLeiturista"
						tabindex="4" size="9"
						onkeypress="javascript:validaEnterComMensagem(event, 'exibirAtualizarRoteiroEmpresaAction.do', 'idLeiturista', 'Leiturista');" />
	                         ...
	                         <% if (indexQuadra++ % 2 == 0) { %>
			                  ...
			                  <% } %>
		                        <td bordercolor="#90c7fc"><div align="center">
		                        	<html:multibox property="idQuadraAdicionar" value="${quadra.id}">
		                        	</html:multibox>
		                        </td>
                ...
                <td colspan="2"><strong> <font color="#FF0000"> 
					<logic:present name="voltar">
						<logic:equal name="voltar" value="filtrar">
							...
							onclick="window.location.href='<html:rewrite page="/exibirFiltrarRoteiroEmpresaAction.do?desfazer=N"/>'">
						</logic:equal>
						<logic:equal name="voltar" value="manter">
							<input name="Button" type="button" class="bottonRightCol"
							value="Voltar" align="left"
							onclick="window.location.href='<html:rewrite page="/exibirManterRoteiroEmpresaAction.do"/>'">
						</logic:equal>
					</logic:present>
                  ...
                  <input type="button" name="Submit22" class="bottonRightCol" value="Desfazer"
                      onclick="window.location.href='<html:rewrite page="/exibirAtualizarRoteiroEmpresaAction.do?desfazer=S"/>'">
				  ...
				  <gsan:controleAcessoBotao name="Button" value="Atualizar"
							  onclick="javascript:validarForm(document.InserirRoteiroEmpresaActionForm);" url="atualizarRoteiroEmpresaAction.do"/>
                </td>
              </tr>
            </table>
