...
function validarForm(){
		...
		if(optsRotas.length >0){
			...
			if(document.getElementById('removeu').value == "nao"){
...
function movimento(direcao) {
	...
	function removerBotao(){
		...
		if (selIndex != -1) {
			...
			document.getElementById('removeu').value = "sim";
...
<html:form action="/informarRotaLeituristaAction"
	...
	type="gsan.gui.micromedicao.InformarRotaLeituristaActionForm"
	method="post">
	
	<input type="hidden" id="removeu" value="nao" />
		...
		<tr>
			...
			<table width="100%" border="0">
				...
				<tr>
					...
					<td>
						<html:text maxlength="4" tabindex="3" property="rota" size="4" readonly="true" />
						<a href="javascript:abrirPopup('exibirPesquisarInformarRotaLeituraAction.do?caminhoRetornoTelaPesquisaRota=exibirPesquisarInformarRotaLeituraAction');"><img width="23" height="21" border="0" src="<bean:message key="caminho.imagens"/>pesquisa.gif" title="Pesquisar Rota" /></a> 
				...
				<logic:notEqual name="InformarRotaLeituristaActionForm" property="leitursitaID" value="-1" >
					...
					<table width="100%">
						<tr>
							...
							onclick="javascript:removerBotao()"  tabindex="6" class="bottonRightCol" /></td>
						</tr>
					</table>
					<table width="100%">
					<!--<tr>
					...
					<tr>					
						<td width="100%">
							<table width="100%">
								<tr>
									<td align="left" colspan="2"><input type="button" name="ButtonDesfazer" class="bottonRightCol" value="Desfazer" 
										tabindex="7" onClick="window.location.href='/gsan/exibirInformarRotaLeituristaAction.do?menu=sim'" />
