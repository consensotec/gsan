  	...
  	if (form.descTarifa.value == ''){
 		...
 		form.descTarifa.focus();
  	    ...
  	    if (validateRequired(form) && validateCaracterEspecial(form) && validateDate(form)){
  	      window.open('exibirManterCategoriaConsumoTarifaAction.do?limpa=1&parametroVigencia='+form.dataVigencia.value+'&parametroDescricao='+form.descTarifa.value+'&parametroIdPerfilLigacao='+form.idLigacaoAguaPerfil.value+'&parametroIdTarifaTipoCalculo='+form.idTarifaTipoCalculo.value+'&parametroSelect=', 'pesquisar','location=no,screenY=0,screenX=0,menubar=no,status=no,toolbar=no,scrollbars=yes,resizable=no,width=630,height=400');
  	    }
  	}
 }
...
<html:form action="/manterConsumoTarifaExistenteAction"
	...
	type="gsan.gui.faturamento.consumotarifa.InserirConsumoTarifaActionForm"
	method="post">
