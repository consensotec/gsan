...
function limparQuantidadeImoveisAtualizados(){
	var form = document.AssociarTarifaConsumoImoveisActionForm;
	form.tarifaAnteriorHidden.value = form.tarifaAnterior.value;
	form.quantidadeImoveisAtualizados.value = "";
}
...
var form = document.AssociarTarifaConsumoImoveisActionForm;
	if ((form.idImovel.value == '') && (!form.idImovel.disabled)){
		form.action = 'exibirAssociarTarifaConsumoImoveisAction.do';
		form.submit();
	}
}
...
function bloquearLSQLS(){
	...
	if(form.idImovel.value != "" ){//se matricula imovel nao for vazio entao deshabilita todos os texts
		...
		form.tarifaAnterior.disabled = true;
		form.tarifaAnterior.value = form.tarifaAnteriorHidden.value;	
		limpar(3);
		
		return false;
	}

	if(form.idImovel.value == ""){//se matricula imovel for vazio entao habilita todos os textos
		...
		form.loteOrigem.disabled = false;
		form.subloteOrigem.disabled = false;
		form.tarifaAnterior.disabled = false;
		//form.tarifaAnterior.value = "";
...
function selecionarQuantidadeDeImoveis(form){
	...
	if(form.tarifaAnterior.value == "" || form.tarifaAnterior.value=="-1"){
		alert('Informe Tarifa Anterior');
	}
	else{
	
	if(form.idImovel.value == ''){
	  	
	   	form.action = 'exibirAssociarTarifaConsumoImoveisAction.do?bloquear=todos&selecionar=sim';
...
function recuperarDadosPopup(codigoRegistro, descricaoRegistro, tipoConsulta) {
	...
	if(tipoConsulta == 'imovel'){
         ...
         document.AssociarTarifaConsumoImoveisActionForm.action = 'exibirAssociarTarifaConsumoImoveisAction.do?bloquear=todos';
         document.AssociarTarifaConsumoImoveisActionForm.submit();
    }
...
function validarForm(form){
	...
	var matricula = trim(form.idImovel.value);
	...
	if (localidadeOrigem.value > localidadeDestino.value){
	...
	if(form.tarifaAtual.value == '-1'){
		alert('Informe Tarifa Atual');
		obrigatorio = true;
		}
		
	// Confirma a validação do formulário
	if (!obrigatorio && validateAssociarTarifaConsumoImoveisActionForm(form)){
		var mensagem = 'Confirmar informação para ';
		mensagem = mensagem + form.quantidadeImoveisAtualizados.value + " imóveis?";
		if (confirm(mensagem)) {
			form.submit();
		}
	}
		...
		var mensagem = 'Confirmar informação para o imóvel ';
	...
	if(!campo.disabled){
	  if (objeto == null || codigoObjeto == null){
	     if(tipo == "" ){
		  ...
		  abrirPopup(url + "?Popup=sim&" + "tipo=" + tipo, altura, largura);
		 }
	 }else{
...
function limparBorrachaOrigem(tipo){
	...
	switch(tipo){
		case 1: //De localidara pra baixo
			...
			form.nomeLocalidadeDestino.value = "";
			form.quantidadeImoveisAtualizados.value = "";
			form.tarifaAnterior.value = "-1";
		   	form.tarifaAtual.value = "-1";
			verificarBotoes();
			desabilitaIntervaloDiferente(1);
			}else{
			break;
...
<script>
    ...
    function validateAssociarTarifaConsumoImoveisActionForm(form) {                                                                   
        if (bCancel) 
      return true; 
        else 
...
<html:javascript staticJavascript="false"
	formName="AssociarTarifaConsumoImoveisActionForm"
...
<html:form action="/associarTarifaConsumoImoveisAction"
	type="gsan.gui.faturamento.consumotarifa.AssociarTarifaConsumoImoveisActionForm"
	name="AssociarTarifaConsumoImoveisActionForm" method="post">
	...
	<table width="770" border="0" cellspacing="5" cellpadding="0">
		<tr>
			...
			<html:hidden property="tarifaAnteriorHidden" />
			...
			<table width="100%" border="0" align="center" cellpadding="0"
				...
				<tr>
					...
					<td class="parabg">Associar Tarifa de Consumo a Imóveis</td>
					<td width="11"><img border="0"
			...
			<table width="100%" border="0">
				<tr>
					<td colspan="3">Para associar a tarifa de consumo, informe o imóvel
					ou o intervalo de inscrição:</td>
					<td align="right"><a href="javascript: abrirPopupHelp('/gsan/help/help.jsp?mapIDHelpSet=faturamentoTarifaConsumoAssociarImoveis', 500, 700);"><span style="font-weight: bold"><font color="#3165CE">Ajuda</font></span></a></td>
				</tr>
				</table>
			<table width="100%" border="0">				
				<tr>
					<td width="23%"><strong> Matricula:</strong></td>
					<td><logic:notEqual parameter="bloquear" value="matricula">
						<html:text maxlength="9" property="idImovel" size="9"
							onkeypress="validaEnterComMensagem(event, 'exibirAssociarTarifaConsumoImoveisAction.do?bloquear=todos', 'idImovel','Matrícula do Imóvel')"
							onkeyup="bloquearLSQLS();limparMatricula();" tabindex="1" />
						<a
							href="javascript:chamarPopup('exibirPesquisarImovelAction.do', '', null, null, 500, 800, '',document.forms[0].idImovel);"><img
						...
						<html:text maxlength="9" property="idImovel" size="9"
							disabled="true"
							onkeypress="validaEnter(event, 'exibirAssociarTarifaConsumoImoveisAction.do?bloquear=todos', 'idImovel')"
								...
								<table width="100%" align="center" bgcolor="#99CCFF">
									...
									<tr bgcolor="#FFFFFF" height="18">
										...
										<div align="center"><span id="endereco"><bean:write
											name="AssociarTarifaConsumoImoveisActionForm"
											property="endereco" />&nbsp;</span></div>
				...
				<tr>
					<logic:equal scope="session" parameter="bloquear" value="todos">
						...
						<td colspan="2"><html:text maxlength="3"
							...
							onkeypress="limparOrigem(1);validaEnterComMensagem(event, 'exibirAssociarTarifaConsumoImoveisAction.do?objetoConsulta=1&inscricaoTipo=origem&bloquear=matricula', 'localidadeOrigemID','Código da Localidade');"
							tabindex="2" onblur="javascript:replicarLocalidade();"
							onkeyup="javascript:bloquearM();limparOrigem(1);" disabled="true"/> <a
							href="javascript:chamarPopup('exibirPesquisarLocalidadeAction.do', 'origem', null, null, 275, 480, '',document.forms[0].localidadeOrigemID);bloquearM();limparOrigem(1);"><img
							...
							<logic:empty name="AssociarTarifaConsumoImoveisActionForm"
							...
							<logic:notEmpty name="AssociarTarifaConsumoImoveisActionForm"
								property="localidadeOrigemID">
								<html:text property="nomeLocalidadeOrigem" size="45"
									readonly="true"
									style="background-color:#EFEFEF; border:0; color: 	#000000"
				...
				<tr>
					...
					<td colspan="2"><html:text maxlength="3"
						...
						onkeypress="limparOrigem(2);validaEnterDependencia(event, 'exibirAssociarTarifaConsumoImoveisAction.do?objetoConsulta=2&inscricaoTipo=origem&bloquear=matricula', document.forms[0].setorComercialOrigemCD, document.forms[0].localidadeOrigemID.value, 'Localidade Inicial');"
						tabindex="3" onblur="javascript:replicarSetorComercial();"
						onkeyup="limparOrigem(2);" disabled="true" /> <a
						href="javascript:chamarPopup('exibirPesquisarSetorComercialAction.do', 'exibirAssociarTarifaConsumoAction', 'idLocalidade', document.AssociarTarifaConsumoImoveisActionForm.localidadeOrigemID.value, 275, 480, 'Informe Localidade Inicial',document.forms[0].setorComercialOrigemCD);
						         limparOrigem(2);"><img
						width="23" height="21" border="0" style="cursor:hand;"
						src="<bean:message key="caminho.imagens"/>pesquisa.gif"
					...
					<img src="<bean:message key="caminho.imagens"/>limparcampo.gif"
						border="0" style="cursor:hand; title=" Apagar Setor Comercial" />
					</a> <html:hidden property="setorComercialOrigemID" /></td>
				</tr>
				<tr>
					<td width="183"><strong>Quadra:</strong></td>

					<td colspan="2" width="432"><html:text maxlength="4"
						property="quadraOrigemNM" size="4"
						onkeypress="limparOrigem(3);validaEnterDependencia(event, 'exibirAssociarTarifaConsumoImoveisAction.do?objetoConsulta=3&inscricaoTipo=origem&bloquear=matricula', document.forms[0].quadraOrigemNM, document.forms[0].setorComercialOrigemCD.value, 'Setor Comercial Inicial');"
						tabindex="4" onblur="replicarQuadra();" disabled="true" /> <logic:present
						name="corQuadraOrigem">
						<logic:equal name="corQuadraOrigem" value="exception">
							<span style="color:#ff0000" id="msgQuadraOrigem"><bean:write
								name="AssociarTarifaConsumoImoveisActionForm"
								property="quadraMensagemOrigem" /></span>
						</logic:equal>
					</logic:present> <html:hidden property="quadraOrigemID" /></td>
				</tr>
				<tr>
					<td><strong>Lote:</strong></td>
					<td colspan="2"><html:text maxlength="4" property="loteOrigem"
						size="3" disabled="true" tabindex="5"
						onkeyup="limparSubloteOrigem();" onblur="replicarLote();" /></td>
				</tr>
				<tr>
					<td><strong>Sublote:</strong></td>
					<td colspan="2"><html:text maxlength="3" property="subloteOrigem"
						size="3" tabindex="6" disabled="true" onkeyup="replicarSublote();"
				...
				<tr>
					...
					<td colspan="2"><html:text maxlength="3"
						...
						onkeypress="validaEnter(event, 'exibirAssociarTarifaConsumoImoveisAction.do?objetoConsulta=1&inscricaoTipo=destino&bloquear=matricula&campoDesabilita=1', 'localidadeDestinoID');"
						tabindex="7" disabled="true"
						onkeyup="limparDestino(1);desabilitaIntervaloDiferente(1);" /> <a
						href="javascript:chamarPopup('exibirPesquisarLocalidadeAction.do', 'destino', null, null, 275, 480, '',form.documents[0].localidadeDestinoID);limparDestino(1);"><img
						width="23" height="21" border="0" style="cursor:hand;"
						src="<bean:message key="caminho.imagens"/>pesquisa.gif"
						title="Pesquisar Localidade" /></a> <logic:present
				...
				<tr>
					...
					<td colspan="2"><html:text maxlength="3"
						...
						onkeypress="validaEnterDependencia(event, 'exibirAssociarTarifaConsumoImoveisAction.do?objetoConsulta=2&inscricaoTipo=destino&bloquear=matricula&campoDesabilita=2', document.forms[0].setorComercialDestinoCD, document.forms[0].localidadeDestinoID.value, 'Localidade Final');"
						tabindex="8" disabled="true"
						onkeyup="limparDestino(2);desabilitaIntervaloDiferente(2);" /> <a
						href="javascript:chamarPopup('exibirPesquisarSetorComercialAction.do', 'exibirAssociarTarifaConsumoAction', 'idLocalidade', document.AssociarTarifaConsumoImoveisActionForm.localidadeDestinoID.value, 275, 480, 'Informe Localidade Final',form.documents[0].setorComercialDestinoCD);
						         limparDestino(2);"><img
						width="23" height="21" border="0" style="cursor:hand;"
						src="<bean:message key="caminho.imagens"/>pesquisa.gif"
						title="Pesquisar Localidade" /></a> <logic:present
					...
					<img src="<bean:message key="caminho.imagens"/>limparcampo.gif"
						border="0" style="cursor:hand; title=" Apagar Setor Comercial" />
					</a> <html:hidden property="setorComercialDestinoID" /></td>
				</tr>
				<tr>
					<td><strong>Quadra:</strong></td>
					<td colspan="2"><html:text maxlength="4" property="quadraDestinoNM"
						size="4"
						onkeypress="validaEnterDependencia(event, 'exibirAssociarTarifaConsumoImoveisAction.do?objetoConsulta=3&inscricaoTipo=destino&bloquear=matricula&campoDesabilita=3', document.forms[0].quadraDestinoNM, document.forms[0].setorComercialDestinoCD.value, 'Setor Comercial Final');"
						tabindex="9" disabled="true"
						onkeyup="limparDestino(3);desabilitaIntervaloDiferente(3);" /> <logic:present
						name="corQuadraDestino">
					...
					</logic:present> <html:hidden property="quadraDestinoID" /></td>
				</tr>
				<tr>
					<td><strong>Lote:</strong></td>
					<td colspan="2"><html:text maxlength="4" property="loteDestino"
						size="3" tabindex="10" disabled="true"
						onkeyup="desabilitaIntervaloDiferente(4);" /></td>
				</tr>
				<tr>
					<td><strong>Sublote:</strong></td>
					<td colspan="2"><html:text maxlength="3" property="subloteDestino"
						size="3" tabindex="11" disabled="true" /></td>
				</tr>
				...
				<logic:notEqual scope="session" parameter="bloquear" value="todos">
					<tr>
						...
						<td colspan="2"><html:text maxlength="3"
							...
							onkeypress="limparOrigem(1);validaEnterComMensagem(event, 'exibirAssociarTarifaConsumoImoveisAction.do?objetoConsulta=1&inscricaoTipo=origem&bloquear=matricula', 'localidadeOrigemID','Código da Localidade');"
							tabindex="2" onblur="javascript:replicarLocalidade();"
							onkeyup="javascript:bloquearM();limparOrigem(1);" /> <a
							href="javascript:chamarPopup('exibirPesquisarLocalidadeAction.do', 'origem', null, null, 275, 480, '',document.forms[0].localidadeOrigemID);
							...
							href="javascript:limparBorrachaOrigem(1);limparMsgQuadraInexistente();document.forms[0].localidadeOrigemID.focus();">
						<img src="<bean:message key="caminho.imagens"/>limparcampo.gif"
							border="0" title="Apagar Localidade" /> </a></td>
					</tr>
					...
					<tr>
						...
						<td colspan="2"><html:text maxlength="3"
							...
							onkeypress="limparOrigem(2);validaEnterDependencia(event, 'exibirAssociarTarifaConsumoImoveisAction.do?objetoConsulta=2&inscricaoTipo=origem&bloquear=matricula', document.forms[0].setorComercialOrigemCD, document.forms[0].localidadeOrigemID.value, 'Localidade Inicial');"
							tabindex="3" onblur="javascript:replicarSetorComercial();"
							onkeyup="limparOrigem(2);" /> <a
							href="javascript:chamarPopup('exibirPesquisarSetorComercialAction.do', 'setorComercialOrigem&Popup=sim', 'idLocalidade', document.AssociarTarifaConsumoImoveisActionForm.localidadeOrigemID.value, 275, 480, 'Informe Localidade Inicial',document.forms[0].setorComercialOrigemCD);
								        limparOrigem(2);"><img
							width="23" height="21" border="0"
							src="<bean:message key="caminho.imagens"/>pesquisa.gif"
							title="Pesquisar Setor Comercial" /></a> <logic:present
							...
							href="javascript:limparBorrachaOrigem(2);document.forms[0].setorComercialOrigemCD.focus();">
						<img src="<bean:message key="caminho.imagens"/>limparcampo.gif"
					...
					<tr>
						...
						<td colspan="2" width="432"><html:text maxlength="4"
							property="quadraOrigemNM" size="4"
							onkeypress="limparOrigem(3);validaEnterDependencia(event, 'exibirAssociarTarifaConsumoImoveisAction.do?objetoConsulta=3&inscricaoTipo=origem&bloquear=matricula', document.forms[0].quadraOrigemNM, document.forms[0].setorComercialOrigemCD.value, 'Setor Comercial Inicial');"
							tabindex="4" onblur="replicarQuadra();" /> <logic:present
							name="corQuadraOrigem">
							<logic:equal name="corQuadraOrigem" value="exception">
								<span style="color:#ff0000" id="msgQuadraOrigem"><bean:write
									name="AssociarTarifaConsumoImoveisActionForm"
					...
					<tr>
						<td><strong>Lote:</strong></td>
						<td colspan="2"><html:text maxlength="4" property="loteOrigem"
							size="3" tabindex="5" onkeyup="limparSubloteOrigem();"
							onblur="replicarLote();" /></td>
					</tr>
					<tr>
						<td><strong>Sublote:</strong></td>
						<td colspan="2"><html:text maxlength="3" property="subloteOrigem"
							size="3" tabindex="6" onblur="replicarSubLote();" /></td>
					</tr>
					<td colspan="3">
					<hr>
					</td>
					...
					<tr>
						...
						<td colspan="2"><html:text maxlength="3"
							...
							onkeypress="limparDestino(1);validaEnter(event, 'exibirAssociarTarifaConsumoImoveisAction.do?objetoConsulta=1&inscricaoTipo=destino&bloquear=matricula&bloquear=matricula&campoDesabilita=1', 'localidadeDestinoID');"
							tabindex="7" onkeyup="desabilitaIntervaloDiferente(1);" /> <a
							href="javascript:chamarPopup('exibirPesquisarLocalidadeAction.do', 'destino', null, null, 275, 480, '',document.forms[0].localidadeDestinoID);
							        limparDestino(1);"><img
							width="23" height="21" border="0"
							src="<bean:message key="caminho.imagens"/>pesquisa.gif"
							title="Pesquisar Localidade" /></a> <logic:present
							...
							href="javascript:limparBorrachaDestino(1);document.forms[0].localidadeDestinoID.focus();">
						<img src="<bean:message key="caminho.imagens"/>limparcampo.gif"
							border="0" title="Apagar Localidade" /> </a></td>
					</tr>
					<tr>
						...
						<td colspan="2"><html:text maxlength="3"
							property="setorComercialDestinoCD" size="3"
							onkeypress="limparDestino(2);validaEnterDependencia(event, 'exibirAssociarTarifaConsumoImoveisAction.do?objetoConsulta=2&inscricaoTipo=destino&bloquear=matricula&campoDesabilita=2', document.forms[0].setorComercialDestinoCD, document.forms[0].localidadeDestinoID.value, 'Localidade Final');"
							tabindex="8" onkeyup="desabilitaIntervaloDiferente(2);" /> <a
							href="javascript:chamarPopup('exibirPesquisarSetorComercialAction.do', 'setorComercialDestino', 'idLocalidade', document.AssociarTarifaConsumoImoveisActionForm.localidadeDestinoID.value, 275, 480, 'Informe Localidade Final',document.forms[0].setorComercialDestinoCD);
							        limparDestino(2);"><img
							width="23" height="21" border="0"
							src="<bean:message key="caminho.imagens"/>pesquisa.gif"
							title="Pesquisar Setor Comercial" /></a> <logic:present
							...
							href="javascript:limparBorrachaDestino(2);document.forms[0].setorComercialDestinoCD.focus();">
						<img src="<bean:message key="caminho.imagens"/>limparcampo.gif"
							border="0" title="Apagar Setor Comercial" /> </a> <html:hidden
					...
					<tr>
						...
						<td colspan="2"><html:text maxlength="4"
							property="quadraDestinoNM" size="4"
							onkeypress="limparDestino(3);validaEnterDependencia(event, 'exibirAssociarTarifaConsumoImoveisAction.do?objetoConsulta=3&inscricaoTipo=destino&bloquear=matricula&campoDesabilita=3', document.forms[0].quadraDestinoNM, document.forms[0].setorComercialDestinoCD.value, 'Setor Comercial Final');"
							tabindex="9" onkeyup="desabilitaIntervaloDiferente(3);" /> <logic:present
							name="corQuadraDestino">
							<logic:equal name="corQuadraDestino" value="exception">
								<span style="color:#ff0000" id="msgQuadraDestino"><bean:write
									name="AssociarTarifaConsumoImoveisActionForm"
					...
					<tr>
						<td><strong>Lote:</strong></td>
						<td colspan="2"><html:text maxlength="4" property="loteDestino"
							size="3" tabindex="10" onkeyup="desabilitaIntervaloDiferente(4);" /></td>
					</tr>
					<tr>
						<td><strong>Sublote:</strong></td>
						<td colspan="2"><html:text maxlength="3" property="subloteDestino"
							size="3" tabindex="11" /></td>
					</tr>
				</logic:notEqual>

				<tr>
					<td><strong>Tarifa Anterior:<font color="#FF0000">*</font></strong></td>
					<td><html:select property="tarifaAnterior"
						onchange="limparQuantidadeImoveisAtualizados();">
				...
				<tr>
					...
					<td><strong>Tarifa Atual:<font color="#FF0000">*</font></strong></td>
					<td colspan="1" align="left"><logic:present
						name="liberarBotaoInserir" scope="request">
						<html:select property="tarifaAtual">
							<html:option value="-1">&nbsp;</html:option>
							<html:options collection="colecaoConsumoTarifa"
								labelProperty="descricao" property="id" />
						</html:select>
						...
						scope="request">
						<html:select property="tarifaAtual" disabled="true">
							<html:option value="-1">&nbsp;</html:option>
							<html:options collection="colecaoConsumoTarifa"
								labelProperty="descricao" property="id" />
						</html:select>
				...
				<tr>
					...
					<td colspan="3" align="right"><logic:empty name="bloquear">
						<input type="button" name="selecionar" disabled="disabled"
							onClick="javascript:selecionarQuantidadeDeImoveis(document.AssociarTarifaConsumoImoveisActionForm);"
							class="bottonRightCol" value="Selecionar">
					</logic:empty> <logic:notEmpty name="bloquear">
				...
				<tr>
					...
					<div align="left"><strong>Quantidade de imóveis que serão
					atualizados: </strong></div>
					</td>
					<td align="right" height="9"><html:text
						property="quantidadeImoveisAtualizados" size="8" maxlength="8"
						readonly="true" /></td>
				</tr>
				<tr>
					<td>&nbsp;</td>
			...
			<table border="0" width="100%">
				<tr>
					<td colspan="1"><input name="Button" type="button"
						class="bottonRightCol" value="Desfazer" align="left"
						onclick="window.location.href='<html:rewrite page="/exibirAssociarTarifaConsumoImoveisAction.do?menu=sim"/>'">
					<input type="button" name="ButtonCancelar" class="bottonRightCol"
						value="Cancelar"
						onClick="javascript:document.forms[0].target='';window.location.href='/gsan/telaPrincipal.do'">
					</td>
					<td colspan="1" align="right"><logic:present
						name="liberarBotaoInserir" scope="request">
