...
<%@ page import="gsan.cadastro.localidade.SetorComercial" %>
<%@ page import="gsan.cadastro.localidade.Quadra" %>
<%@ page import="gsan.micromedicao.Rota" %>
...
<html:javascript staticJavascript="false"  formName="AtualizarContaComunicadoActionForm" />
...
function desabilitarGrupoFaturamento(){
    ...
    if (form.gerenciaRegional.value != ""){
      tirarSelecaoListBoxSelecionado(form.grupoFaturamento);
...
function validaForm(){
	var form = document.forms[0];
	if (validateAtualizarContaComunicadoActionForm(form)){
 ...
 function recuperarDadosPopup(codigoRegistro, descricaoRegistro, tipoConsulta) {
	...
	if (tipoConsulta == "localidade") {
      ...
      form.action = "exibirAtualizarContaComunicadoAction.do"
 	  submeterFormPadrao(form);
			...
			for ( i = 0; i < form.setorComercial.length; i++) {
				if ( form.setorComercial[i].selected ) {
					if ( form.setorComercial[i].value != "" ) {
						...
						form.action = 'exibirAtualizarContaComunicadoAction.do?acao=rota&id='+obj;
						form.submit();
					} else {
						form.action = 'exibirAtualizarContaComunicadoAction.do';
						form.submit();
			...
			for ( i = 0; i < form.rota.length; i++) {
				if ( form.rota[i].selected ) {
					if ( form.rota[i].value != "" ) {
						...
						form.action = 'exibirAtualizarContaComunicadoAction.do?acao=quadra&id='+obj;
						form.submit();
					} else {
						form.action = 'exibirAtualizarContaComunicadoAction.do';
						form.submit();
...
function tirarSelecaoListBoxSelecionado(listBox){
	
	retorno = false;
	
	for ( i = (listBox.length -1); i >= 0; i--){
		if (listBox.options[i].value.length > 0 && listBox.options[i].value != "-1" && listBox.options[i].selected == true) {
...
<html:form action="/atualizarContaComunicadoAction.do"
	name="AtualizarContaComunicadoActionForm"
	type="gsan.gui.faturamento.conta.AtualizarContaComunicadoActionForm"
	method="post" onsubmit="return validateAtualizarContaComunicadoActionForm(this);" >
			...
			<td width="600" valign="top" class="centercoltext">
				...
				<table width="100%" border="0" align="center" cellpadding="0"
					...
					<tr>
						...
						<td class="parabg">Atualizar Comunicado na Conta</td>
						<td width="11"><img border="0"
				...
				<table border="0" width="100%">
					<tr>
						<td colspan="3">Para atualizar o comunicado na conta, informe os
						dados abaixo:</td>
						<logic:present scope="application" name="urlHelp">
							<td align="right"><a href="javascript:abrirPopupHelp('${applicationScope.urlHelp}faturamentoContaComunicado', 500, 700);"><span style="font-weight: bold"><font color="#3165CE">Ajuda</font></span></a></td>									
						</logic:present>
				...
				<table border="0" width="100%">	
						...
						<td align="right" colspan="2">
							<div align="left">
								<html:text property="referencia"
											...
											onkeypress="return isCampoNumerico(event);" disabled="true"/>
								mm/aaaa
							</div>
						</td>
					</tr>
									...
									<tr>
										...
										<td>
											<html:text property="localidade" size="3"
														maxlength="3" 
														onkeypress="validaEnter(event, 'exibirAtualizarContaComunicadoAction.do', 'localidade');return isCampoNumerico(event);" />
					...
					<tr>
						...
						<td align="right">
							...
							value="Atualizar" onclick="validaForm();" type="button">
						</td>
					</tr>
				</table>
				<p>&nbsp;</p>
