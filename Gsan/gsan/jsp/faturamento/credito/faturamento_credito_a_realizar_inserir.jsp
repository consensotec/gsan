...
function validarForm(form){
	...
	if(validateInserirCreditoARealizarActionForm(form)){
		...
		} else if(form.mesAnoInicialDoCredito.value == "" ){
			alert("Mês/Ano Inicial da Cobrança não pode ser vazio"); 


		}else if (!validaAnoMesSemAlert(form.mesAnoInicialDoCredito)) {
			alert("Mês/Ano Inicial do Crédito invalido!");
				

		} else {		
			sMes1 = form.mesAnoInicialDoCredito.value.substr(0, 2);
			sAno1 = form.mesAnoInicialDoCredito.value.substr(3, 7);
			smesAno1 = sAno1 + sMes1;

			sMes2 = form.valorMinimoMesAnoInicialDoCredito.value.substr(0, 2);
			sAno2 = form.valorMinimoMesAnoInicialDoCredito.value.substr(3, 7);
			smesAno2 = sAno2 + sMes2;

			if (parseInt(smesAno1) < parseInt(smesAno2)) {
				alert("O mês/ano inicial da crédito não pode ser inferior "
						+ form.valorMinimoMesAnoInicialDoCredito.value);
...
<div id="formDiv"><html:form action="/exibirInserirCreditoARealizarAction" method="post">
	...
	<html:hidden property="valorMinimoMesAnoInicialDoCredito"/>
	...
	<table width="770" border="0" cellspacing="5" cellpadding="0">
		<tr>
			...
			<table width="100%" border="0">
				...
				<tr>
					<td><strong>Mês/Ano Inicial do Crédito:</strong><font color="#FF0000">*</font></td>
					<td colspan="2" align="right">
						<div align="left">
							<html:text property="mesAnoInicialDoCredito" maxlength="7"
