...
<%@ page import="gsan.faturamento.debito.DebitoACobrar,gsan.util.Util"%>
...
$(document).ready(function(){
	...
	var ra = form.indicadorPermiteCancelarDebito.value;
	if (ra==1){
		//Exibe RA (RM5759)
		document.getElementById('ra').style.visibility = 'visible'; 
		document.getElementById('ra').style.position = 'relative';
		if (form.usuarioPermissaoEspecial.value==1){//Retira obrigatoriedade do RA
			document.getElementById('obgRA').style.visibility = 'hidden';			
		} else {
			document.getElementById('obgRA').style.visibility = 'visible';
		}
	} else {
	...
	if (form.idRegistroAtendimento.value!=''){
		form.codigoImovel.disabled=1;
	}
	if(form.descricaoEspecificacaoRA.value==''){
		form.idRegistroAtendimento.value='';
		form.codigoImovel.disabled=0;
	}
});
...
function reloadRA() {
	var form = document.forms[0];
	form.action = "/gsan/exibirManterDebitoACobrarAction.do?pesquisarRA=SIM";
...
// RM 5759
function validaRA(form){
	if (form.usuarioPermissaoEspecial.value==1 || form.indicadorPermiteCancelarDebito.value==2){
		return true;
	} else {
		var idRA = form.idRegistroAtendimento;
		if (idRA.value ==""){
			alert("É obrigatório o preenchimento do RA para o cancelamento do débito.");
			return false;	
		} else {
			if (form.registroAtendimentoCancelaDebito.value==2){
				alert("O tipo de solicitação do Registro de Atendimento n. "+idRA.value+" não permite cancelamento de débito.");
				return false;
			} else {
				return true;
			}
...
<html:form action="/exibirManterDebitoACobrarAction.do"
	...
	type="gsan.gui.faturamento.ManterDebitoACobrarActionForm" method="post">
	...
	<table width="770" border="0" cellspacing="5" cellpadding="0">
		<tr>
			...
			<!-- RM 5759 -->
			<div id="ra" style="visibility:hidden; position:absolute">
							
			<table width="100%" border="0">
			<tr>
				<td WIDTH="120"><strong>Número do RA:<font id="obgRA" color="#FF0000">*</font></strong></td>
				<td>
					<html:text property="idRegistroAtendimento" 
								size="9" maxlength="9" tabindex="1"
								onkeypress="validaEnterComMensagem(event, 'exibirManterDebitoACobrarAction.do?pesquisarRA=SIM', 'idRegistroAtendimento', 'Registro de Atendimento');return isCampoNumerico(event);"
					...
					<logic:notPresent name="corRA">
						<logic:empty name="ManterDebitoACobrarActionForm" property="idRegistroAtendimento">
							<html:text property="descricaoEspecificacaoRA" 
									   value="" 
									   size="45" 
									   readonly="true"
						...
						<logic:notEmpty name="ManterDebitoACobrarActionForm" property="idRegistroAtendimento">
							<html:text property="descricaoEspecificacaoRA" 
									   size="45" 
									   readonly="true" 
									   style="background-color:#EFEFEF; border:0; color: #000000" />
			...
			<html:text maxlength="1" property="indicadorPermiteCancelarDebito" size="1" style="visibility:hidden"/>
			<html:text maxlength="1" property="usuarioPermissaoEspecial" size="1" style="visibility:hidden"/>
			<html:text maxlength="1" property="registroAtendimentoCancelaDebito" size="1" style="visibility:hidden"/>
			</div> 
