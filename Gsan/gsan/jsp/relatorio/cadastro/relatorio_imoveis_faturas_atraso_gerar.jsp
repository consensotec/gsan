		...
		if(form.criterioFiltro[i].checked){
			if(form.criterioFiltro[i].value == "2"){
				if(form.codigoClienteSuperior.value.length > 0) {
					...
					desabilitarClienteResponsavelTiposCliente();
...
function limparCamposClienteAtravesBorracha(tipo){
    ...
    if(tipo == 'cliente' && !form.codigoCliente.readOnly && !form.codigoCliente.disabled){
    	...
    	form.nomeCliente.value = "";
		habilitarCamposCliente();
		form.tiposCliente.disabled = false;
	}
	...
	function desabilitarClienteResponsavelTiposCliente(){
		var form = document.forms[0];
		
		form.tiposCliente.value = <%=""+ConstantesSistema.NUMERO_NAO_INFORMADO%>;
		form.tiposCliente.disabled = true;
	...
	function validarValorMonetario(campo, nomeCampo){
		var regExValorMonetario = /^[0-9.,]*$/;
		
		if(!regExValorMonetario.test(campo.value)){
			alert(nomeCampo + ' deve conter apenas números, pontos e vírgula.');
			return false;	
		}else{
			return true;
		}
	...
	function validarCliente(){		
		...
		if(form.criterioFiltro[1].checked){
			...
			var tipoCliente = form.tiposCliente.value;

			if(clienteSuperior == '' && cliente == '' && tipoCliente == ''){	
					...
					if( msg != ""){
						...
						if( msg != ""){
							...
							if( msg != ""){
								...
								msg = verificarAtributosInicialEFinal(form.valorFaturasAtrasoInicial,form.valorFaturasAtrasoFinal,"Valor de Faturas");
								if( msg != ""){                                                      
									alert(msg);
									return false;
								}			
...
function verificarAtributosInicialEFinal(campoInicio, campoFim, nomeCampo){
	...
	function limparBorrachaOrigem(tipo){
		...
		switch(tipo){
			...
			case 3: //De localidara pra baixo
	...
	function limparBorrachaDestino(tipo){
      ...
      if(!form.criterioFiltro[1].checked){		
		switch(tipo){
			...
			case 3: //De localidade pra baixo
				form.localidadeFinal.value = "";
				form.nomeLocalidadeFinal.value = "";
				form.setorComercialFinal.value = "";
			    habilitarCamposLocalizacaoGeografica();
   ...
   function validarDiferentes(){
     ...
     var rotaInicial = form.rotaInicial.value;
     var rotaFinal = form.rotaFinal.value;
     var setorInicial = form.setorComercialInicial.value;
     var setorFinal = form.setorComercialFinal.value;
     var localidadeInicial = form.localidadeInicial.value;
     var localidadeFinal = form.localidadeFinal.value;
     var qtd =1;
     if( rotaInicial != rotaFinal){
         form.sequencialRotaInicial.disabled = true;
         form.sequencialRotaFinal.disabled = true;           
       }else if(setorInicial != setorFinal ){
         form.rotaInicial.disabled = true;
         form.rotaFinal.disabled = true;
         form.sequencialRotaInicial.disabled = true;
         form.sequencialRotaFinal.disabled = true;
       }else if(localidadeInicial !=localidadeFinal){
    	   form.rotaInicial.disabled = true;
           form.rotaFinal.disabled = true;
           form.sequencialRotaInicial.disabled = true;
           form.sequencialRotaFinal.disabled = true;
...
function reloadForm(){
    ...
    function replicarQuadra(){
		...
		formulario.quadraInicial.focus;
	}
...
<body leftmargin="5" topmargin="5" onload="javascript:setarFoco('${requestScope.mesAnoFaturamento}');
	desabilitarCamposDeAcordoCriterioFiltro(false);validarDiferentes();desabilitarClienteOuClienteSuperior();marcarHidrometro(); toggleBox('divAux',0);">

<div id="formDiv">
<html:form action="/gerarRelatorioImoveisFaturasAtrasoAction.do"
	name="GerarRelatorioImoveisFaturasAtrasoActionForm"
	type="gsan.gui.relatorio.cadastro.GerarRelatorioImoveisFaturasAtrasoActionForm"
	method="post">
			...
			<table width="100%" border="0">
				...
				<tr>
					...
					<td>
						...
						<a href="javascript:chamarPopup('exibirPesquisarLocalidadeAction.do', 'origem', null, null, 275, 480, '',document.forms[0].localidadeInicial);limparOrigem(1);"><img width="23" 
								height="21" 
								border="0" 
								style="cursor:hand;"
				...
				<tr>
					...
					<td colspan="2">
						...
						<a href="javascript:habilitarPesquisaClienteSuperior();" title="Pesquisar Cliente Superior"><img width="23" height="21" src="<bean:message key='caminho.imagens'/>pesquisa.gif" border="0" /></a>

						<logic:present name="codigoClienteSuperiorEncontrado" scope="request">
								<html:text property="nomeClienteSuperior" size="34"	readonly="true"
									style="background-color:#EFEFEF; border:0; color: #000000" />
						</logic:present> 
				...
				<tr>
					...
					<td colspan="3">
						...
						<a href="javascript:habilitarPesquisaCliente(document.forms[0]);" title="Pesquisar Cliente"><img width="23" height="21" src="<bean:message key='caminho.imagens'/>pesquisa.gif" border="0" /></a>
						
						<logic:present name="codigoClienteEncontrado" scope="request">
								<html:text property="nomeCliente" size="34"	readonly="true"
									style="background-color:#EFEFEF; border:0; color: #000000" />
						</logic:present> 
						...
						<a href="javascript:limparCamposClienteAtravesBorracha('cliente');" title="Apagar"> 
			  ...
			  <tr> 
               ...
               <strong>Tipo de cliente:</strong>                		
                	...
                	<td colspan="6">
               			<strong>
						<html:select property="tiposCliente" 
							style="width: 350px; height:100px;" 
							multiple="true">
							
							<html:option value="<%=""+ConstantesSistema.NUMERO_NAO_INFORMADO%>">&nbsp;</html:option>
							
							<logic:present property="colecaoTiposCliente" name="GerarRelatorioImoveisFaturasAtrasoActionForm">
								
								<html:optionsCollection property="colecaoTiposCliente" 
									label="descricao" value="id" />
							</logic:present>
						</html:select>                
                 		</strong>
              		...
              		<td colspan="6">
              			<strong>
	              			<html:select property="grupoCobranca">
								...
								<logic:present name="GerarRelatorioImoveisFaturasAtrasoActionForm" property="colecaoGruposCobranca">
								   <html:optionsCollection name="GerarRelatorioImoveisFaturasAtrasoActionForm" property="colecaoGruposCobranca" label="descricao" value="id"/>
								</logic:present>
	              			</html:select>
              			</strong>
              		</td>
				...
				<tr>
					...
					<td colspan="3">					
						<html:text maxlength="10"
							...
							onkeyup="javascript:replicarCampo( document.forms[0].valorFaturasAtrasoFinal, document.forms[0].valorFaturasAtrasoInicial ); formataValorMonetario(this); formataValorMonetario(document.forms[0].valorFaturasAtrasoFinal);"/>
						<strong>a</strong>
						<html:text maxlength="10"
							size="10"
							tabindex="1"
							...
							onkeyup="formataValorMonetario(this);" />
					</td>
				</tr>
				<tr>
