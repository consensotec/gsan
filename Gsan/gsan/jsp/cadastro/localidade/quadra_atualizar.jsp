...
<%@ page import="gsan.cadastro.localidade.QuadraFace"%>
...
<html:javascript staticJavascript="true"
	formName="AtualizarQuadraActionForm" />
...
<!--
	...
	function atualizarDadosIndicador(formulario) {
		var objIndicadorRedeAgua = returnObject(formulario,
				"indicadorRedeAguaAux");
		var objIndicadorRedeEsgoto = returnObject(formulario,
				"indicadorRedeEsgotoAux");
	...
	function validarIndicadorRedeAguaAux() {
		...
		for (indice = 0; indice < form.elements.length; indice++) {
			if (form.elements[indice].type == 'radio'
					...
					&& formulario.elements[indice].name == "indicadorRedeAguaAux") {
	...
	function validarIndicadorRedeEsgotoAux() {
		...
		for (indice = 0; indice < form.elements.length; indice++) {
			if (form.elements[indice].type == 'radio'
					&& form.elements[indice].checked == true
					&& formulario.elements[indice].name == "indicadorRedeEsgotoAux") {
				selecionado = form.elements[indice].value;
				indice = form.elements.length;
			}
		}
	...
	<!--
		function validarForm(formulario) {
			...
			if (validateAtualizarQuadraActionForm(formulario)) {
				var objSistemaEsgotoID = returnObject(formulario,
						"sistemaEsgotoID");
				var objBaciaID = returnObject(formulario, "baciaID");
				var objDistritoOperacionalID = returnObject(formulario,
						"distritoOperacionalID");
				var objSetorCensitarioID = returnObject(formulario,
						"setorCensitarioID");
				var objRotaID = returnObject(formulario, "rotaID");
				var objPerfilQuadra = returnObject(formulario, "perfilQuadra");

				var objIndicadorAguaHTML = document
						.getElementById("indicadorRedeAguaHTML");
				var objIndicadorEsgotoHTML = document
						.getElementById("indicadorRedeEsgotoHTML");
				...
				//com rede ou parcial
				if (document.forms[0].indicadorRedeEsgotoAux[1].checked
						|| document.forms[0].indicadorRedeEsgotoAux[2].checked) {
					...
					if (objSistemaEsgotoID.options[objSistemaEsgotoID.options.selectedIndex].value == "-1") {
						...
						alert("Informe Bacia.");
						objSistemaEsgotoID.focus();
					} else if (form.indicadorRelacionamentoQuadraBairro.value == "1"
							&& form.bairroID.value == "") {
						alert("Informe Bairro.");
						objBairroID.focus();
					}
					//com rede ou parcial
					else if (document.forms[0].indicadorRedeAguaAux[1].checked
							|| document.forms[0].indicadorRedeAguaAux[2].checked) {
						if (objDistritoOperacionalID.value.length < 1) {
							alert("Informe Distrito Operacional.");
							objDistritoOperacionalID.focus();
						} else if (objDistritoOperacionalID.value.length > 0
								&& !testarCampoValorZero(
										objDistritoOperacionalID,
										"Distrito Operacional")) {
							objDistritoOperacionalID.focus();
						} else if (objSetorCensitarioID.value.length > 0
								&& !testarCampoValorZero(objSetorCensitarioID,
										"Setor Censitário")) {
							objSetorCensitarioID.focus();
						} else if (objRotaID.value.length < 1) {
							alert("Informe Rota.");
							objRotaID.focus();
						} else if (objRotaID.value.length > 0
								&& !testarCampoValorZero(objRotaID, "Rota")) {
							objRotaID.focus();
							...
							&& !testarCampoValorZero(objDistritoOperacionalID,
									"Distrito Operacional")) {
						objDistritoOperacionalID.focus();
					} else if (objSetorCensitarioID.value.length > 0
							&& !testarCampoValorZero(objSetorCensitarioID,
		...
		function validateAtualizarQuadraActionForm(form) {
			if (bCancel)
				...
				return validateCaracterEspecial(form) && validateRequired(form)
						&& validateInteiroZeroPositivo(form)
						&& validateLong(form);
		}

		function caracteresespeciais() {
			this.aa = new Array("rotaID", "Rota possui caracteres especiais.",
					new Function("varName", " return this[varName];"));
			this.ab = new Array("setorCensitarioID",
					"Setor Censitário possui caracteres especiais.",
					new Function("varName", " return this[varName];"));
			this.ac = new Array("distritoOperacionalID",
					"Distrito Operacional possui caracteres especiais.",
					new Function("varName", " return this[varName];"));
		}
		...
		function InteiroZeroPositivoValidations() {
			this.aa = new Array("rotaID",
					"Rota deve somente conter números positivos ou zero.",
					new Function("varName", " return this[varName];"));
		}

		function IntegerValidations() {
			this.aa = new Array("setorCensitarioID",
					"Setor Censitário deve somente conter números positivos.",
					new Function("varName", " return this[varName];"));
			this.ab = new Array(
					"distritoOperacionalID",
					"Distrito Operacional deve somente conter números positivos.",
					new Function("varName", " return this[varName];"));
		}
	//-->
	</SCRIPT>
	...
	<!--
		function validarForm(formulario) {
			...
			if (validateAtualizarQuadraActionForm(formulario)) {
				...
				var permissaoAdicionarQuadraFace = document
						.getElementById("permissaoAdicionarQuadraFace");
				var quadraFaceInformada = document
						.getElementById("quadraFaceInformada");

				if (permissaoAdicionarQuadraFace.value == "1"
						&& quadraFaceInformada.value == "2") {
		...
		function removerQuadraFace(numeroQuadraFace) {
			...
			if (confirm("Confirma remoção?")) {
				...
				form.action = "/gsan/exibirAtualizarQuadraAction.do?numeroQuadraFace="
						+ numeroQuadraFace;
...
<logic:equal name="permissaoAdicionarQuadraFace" value="2">
	...
	<body leftmargin="5" topmargin="5"
		onload="atualizarDadosIndicador(document.forms[0]);
...
<input type="hidden" id="permissaoAdicionarQuadraFace"
	name="permissaoAdicionarQuadraFace"
...
<div id="formDiv">
	...
	<html:form action="/exibirAtualizarQuadraAction"
		...
		type="gsan.gui.cadastro.localidade.AtualizarQuadraActionForm"
		method="post">
		...
		<table width="770" border="0" cellspacing="5" cellpadding="0">
			<tr>
				...
				<td width="625" valign="top" class="centercoltext">
					...
					<table width="100%" border="0">
						<tr>
							...
							<logic:present scope="application" name="urlHelp">
								<td align="right"><a
									href="javascript:abrirPopupHelp('${applicationScope.urlHelp}cadastroLocalizacaoQuadraAtualizar', 500, 700);"><span
					...
					<table width="100%" border="0">
										...
										<td width="100%" align="center">
											...
											<table width="100%" border="0">
												<tr>
													...
													<td><html:hidden property="localidadeID" /> <bean:write
															name="AtualizarQuadraActionForm" property="localidadeID" />
														- <bean:write name="AtualizarQuadraActionForm"
															property="localidadeNome" /></td>
												</tr>
												<tr>
													...
													<td><html:hidden property="setorComercialCD" /> <bean:write
															name="AtualizarQuadraActionForm"
															property="setorComercialCD" /> - <bean:write
															name="AtualizarQuadraActionForm"
															property="setorComercialNome" /> <html:hidden
															property="setorComercialID" /></td>
												</tr>
												<tr>
													<td><strong>Quadra:<font color="#FF0000">*</font>
													</strong>
													</td>
													<td><bean:write name="AtualizarQuadraActionForm"
															property="quadraNM" /></td>
												</tr>

												<tr>
													<td><strong>Rota:<font color="#FF0000">*</font>
													</strong>
													</td>
													<td><html:hidden property="rotaID" /> <html:text
															property="codigoRota" size="5" maxlength="4" tabindex="1"
													...
													</a> <logic:present name="corRotaMensagem">

															<logic:equal name="corRotaMensagem" value="exception">
																<html:text property="rotaMensagem" size="45"
																	readonly="true"
												...
												<tr>
													...
													<td><html:text property="bairroID" maxlength="4"
														...
														<a
														href="javascript: abrirPopup('exibirPesquisarBairroAction.do?idMunicipio='+document.forms[0].municipioID.value, 400, 800);">
															<img width="23" height="21" border="0"
															src="<bean:message key="caminho.imagens"/>pesquisa.gif"
															title="Pesquisar Bairro" />
													</a> <logic:present name="corBairro">
															...
															<logic:notEqual name="corBairro" value="exception">
																<html:text property="bairroDescricao" size="45"
																	readonly="true"
																	style="background-color:#EFEFEF; border:0; color: #000000" />
															</logic:notEqual>
															...
															<logic:empty name="AtualizarQuadraActionForm"
																property="bairroID">
																<html:text property="bairroDescricao" size="30" value=""
																	readonly="true"
																	style="background-color:#EFEFEF; border:0; color: #ff0000" />
															</logic:empty>
														...
														</logic:notPresent> <a href="javascript:limpar(13);"> <img
															src="<bean:message key="caminho.imagens"/>limparcampo.gif"
															border="0" title="Apagar" /> </a></td>
												</tr>
						...
						<tr>
							<td colspan="2">
								...
								<table width="100%" align="center" bgcolor="#90c7fc" border="0">
									<tr>
										<td><strong>Características</strong>
										</td>
									</tr>
									<tr bgcolor="#cbe5fe">
										<td width="100%" align="center">
											...
											<table width="100%" border="0">
												<tr>
													<td width="20%"><strong>Incrementar Lote:</strong>
													</td>
													<td><html:radio property="indicadorIncrementoLote"
															value="1" tabindex="8" /><strong>Sim</strong> <html:radio
															property="indicadorIncrementoLote" value="2" tabindex="9" /><strong>N&atilde;o</strong>
													</td>
												</tr>
												<tr>
													<td><strong>Tipo da Área:</strong>
													</td>
													<td><html:select property="areaTipoID"
												...
												<tr>
													<td><strong>Perfil da Quadra:<font
															color="#FF0000">*</font>
													</strong>
													</td>
													<td><html:select property="perfilQuadra"
												...
												<logic:equal name="permissaoAdicionarQuadraFace" value="2">
													...
													<INPUT TYPE="hidden" NAME="indicadorRedeAguaHTML"
														ID="indicadorRedeAguaHTML">

													<INPUT TYPE="hidden" NAME="indicadorRedeEsgotoHTML"
														ID="indicadorRedeEsgotoHTML">

													<tr>
														<td colspan="2" height="5"></td>
													</tr>
													<tr>
														<td><strong>Rede de Água:<font
																color="#FF0000">*</font>
														</strong>
														</td>
														<td><html:radio property="indicadorRedeAguaAux"
																value="1" tabindex="6"
																onclick="verificarMarcacao(1, 'Agua');habilitacaoDistritoOperacional(document.forms[0].indicadorRedeAguaAux.value);" /><strong>Sem
																Rede de Água <html:radio property="indicadorRedeAguaAux"
																	value="2" tabindex="6"
																	onclick="verificarMarcacao(2, 'Agua');habilitacaoDistritoOperacional(document.forms[0].indicadorRedeAguaAux.value);" />Com
																Rede de Água <html:radio property="indicadorRedeAguaAux"
																	value="3" tabindex="7"
																	onclick="verificarMarcacao(3, 'Agua');habilitacaoDistritoOperacional(document.forms[0].indicadorRedeAguaAux.value);" />Rede
																de Água Parcial</strong></td>
													</tr>
													<tr>
														<td><strong>Rede de Esgoto:<font
																color="#FF0000">*</font>
														</strong>
														</td>
														<td><html:radio property="indicadorRedeEsgotoAux"
																value="1" tabindex="8"
																onclick="verificarMarcacao(1, 'Esgoto');habilitacaoSistemaEsgotoBacia(document.forms[0].indicadorRedeEsgotoAux.value);" /><strong>Sem
																Rede de Esgoto <html:radio
																	property="indicadorRedeEsgotoAux" value="2"
																	tabindex="9"
																	onclick="verificarMarcacao(2, 'Esgoto');habilitacaoSistemaEsgotoBacia(document.forms[0].indicadorRedeEsgotoAux.value);" />Com
																Rede de Esgoto <html:radio
																	property="indicadorRedeEsgotoAux" value="3"
																	tabindex="10"
																	onclick="verificarMarcacao(3, 'Esgoto');habilitacaoSistemaEsgotoBacia(document.forms[0].indicadorRedeEsgotoAux.value);" />Rede
																de Esgoto Parcial</strong></td>
													</tr>

													<tr>
														<td><strong>Sistema Esgoto:<font
																color="#FF0000">*</font>
														</strong>
														</td>
														<td><html:select property="sistemaEsgotoID"
																style="width: 200px;" tabindex="13"
																onchange="redirecionarSubmit('exibirAtualizarQuadraAction.do?objetoConsulta=7');">
																<html:option
																	value="<%=""+ConstantesSistema.NUMERO_NAO_INFORMADO%>">&nbsp;</html:option>
													...
													<tr>
														<td><strong>Bacia:<font color="#FF0000">*</font>
														</strong>
														</TD>
														<td><html:select property="baciaID"
													...
													<tr>
														...
														<td><html:text property="distritoOperacionalID"
																size="5" maxlength="3" tabindex="15"
																...
																onkeyup="limpar(10);" /> <a
															href="javascript:abrirPopup('exibirPesquisarDistritoOperacionalAction.do?limparForm=sim', 250, 495);">
																<img width="23" height="21" border="0"
																src="<bean:message key="caminho.imagens"/>pesquisa.gif"
																title="Pesquisar Distrito Operacional" />
														</a> <logic:present name="corDistritoOperacional">

																<logic:equal name="corDistritoOperacional"
																	value="exception">
																...
																<logic:notEqual name="corDistritoOperacional"
																	value="exception">
																	<html:text property="distritoOperacionalDescricao"
																		size="45" readonly="true"
																		style="background-color:#EFEFEF; border:0; color: #000000" />
												...
												<tr>
													<td><strong>Quadra Criada para Atualização
															Cadastral? <font color="FF0000">*</font>
													</strong>
													</td>
													<td><html:radio
															property="indicadorAtualizacaoCadastral" value="1"
															tabindex="6" /><strong> Sim <html:radio
																property="indicadorAtualizacaoCadastral" value="2"
																tabindex="6" /> Não </strong></td>
								...
								<table width="100%" align="center" bgcolor="#90c7fc" border="0">
									...
									<tr bgcolor="#cbe5fe">
										<td width="100%" align="center">
											...
											<table width="100%" border="0">
												<tr>
													<td><strong>Setor Censitário:</strong>
													</td>
													<td><html:text property="setorCensitarioID" size="5"
															maxlength="3" tabindex="16" onkeyup="limpar(11);"
														...
														href="javascript:abrirPopup('exibirPesquisarTabelaAuxiliarAction.do?tela=ibgeSetorCensitario',275,480);">
															<img width="23" height="21" border="0"
															src="<bean:message key="caminho.imagens"/>pesquisa.gif"
															title="Pesquisar Setor Censitário" />
													</a> <logic:present name="corSetorCensitario">
															...
															<logic:notEqual name="corSetorCensitario"
																value="exception">
																<html:text property="setorCensitarioDescricao" size="45"
																	readonly="true"
																	style="background-color:#EFEFEF; border:0; color: #000000" />
												...
												<tr>
													<td><strong>ZEIS:</strong>
													</td>
													<td><html:select property="zeisID"
															style="width: 200px;" tabindex="17">
															<html:option
																value="<%=""+ConstantesSistema.NUMERO_NAO_INFORMADO%>">&nbsp;</html:option>
						...
						<logic:equal name="permissaoAdicionarQuadraFace" value="1">
							...
							<tr>
								<td colspan="2">
									...
									<table width="100%" border="0">
										<tr>
											<td height="17" colspan="3"><strong>Face(s) da
													quadra:<font color="#FF0000">*</font>
											</strong>
											</td>
											<td align="right"><input type="button"
												class="bottonRightCol" value="Adicionar" tabindex="11"
												style="width: 80px"
												onclick="javascript:abrirPopup('exibirAdicionarQuadraFaceAction.do?acao=inserir&telaRetorno=exibirAtualizarQuadraAction', 295, 450);">
											</td>
										</tr>
										...
										<logic:present name="colecaoQuadraFace">
											...
											<logic:notEmpty name="colecaoQuadraFace">
												...
												<input type="hidden" id="quadraFaceInformada"
													name="quadraFaceInformada" value="1">
																...
																<td>
																	...
																	<table width="100%" bgcolor="#99CCFF">
																		<tr bgcolor="#99CCFF">
																			...
																			<td width="30%">
																				<div align="center">
																					<strong>Distrito Opereracional</strong>
																				</div></td>
																			<td width="30%">
																				<div align="center">
																					<strong>Bacia</strong>
																...
																<td>
																	...
																	<div style="width: 100%; height: 100; overflow: auto;">
																		...
																		<table width="100%" align="center" bgcolor="#99CCFF">
																			...
																			<logic:iterate name="colecaoQuadraFace"
																				id="quadraFace" type="QuadraFace">
																				...
																				<tr bgcolor="#FFFFFF">
																					...
																					<td align="center" width="10%" valign="middle">
																						<a
																						href="javascript:removerQuadraFace('<%=""
											+ quadraFace.getNumeroQuadraFace()
													.intValue()%>')">
																							<img
																							src="<bean:message key='caminho.imagens'/>Error.gif"
																							border="0"> </a></td>
																					<td width="10%" align="center"><a
																						href="javascript:abrirPopup('exibirAdicionarQuadraFaceAction.do?acao=atualizar&telaRetorno=exibirAtualizarQuadraAction&numeroQuadraFace=<%=""
											+ quadraFace.getNumeroQuadraFace()
													.intValue()%>',400, 630);">
																							<bean:write name="quadraFace"
																								property="numeroQuadraFace" />
																					</a>
																					</td>
																					<td width="10%" align="center"><bean:write
																							name="quadraFace" property="descricaoRedeAgua" />
																					</td>
																					<td width="10%" align="center"><bean:write
																							name="quadraFace" property="descricaoRedeEsgoto" />
																					</td>
																					<td width="30%"><logic:present
																							name="quadraFace" property="distritoOperacional">
																							<bean:write name="quadraFace"
																								property="distritoOperacional.descricaoAbreviada" />
																						</logic:present> <logic:notPresent name="quadraFace"
																							property="distritoOperacional">
														&nbsp;
													</logic:notPresent></td>

																					<td width="30%"><logic:present
																							name="quadraFace" property="bacia">
																							<bean:write name="quadraFace"
																								property="bacia.descricao" />
																						</logic:present> <logic:notPresent name="quadraFace"
																							property="bacia">
														&nbsp;
													</logic:notPresent></td>
											...
											<logic:empty name="colecaoQuadraFace">
												...
												<input type="hidden" id="quadraFaceInformada"
													name="quadraFaceInformada" value="2">
						...
						<tr>
							<td height="30" width="20%"><strong>Indicador de
									uso:</strong>
							</td>
							<td><html:radio property="indicadorUso" value="1" /><strong>Ativo
									<html:radio property="indicadorUso" value="2" />Inativo</strong></td>
						</tr>
						...
						<logic:equal name="pemissaoIndicadorBloqueio" value="1">
							<tr>
								...
								<td><html:radio property="indicadorBloqueio" value="1" /><strong>Sim
										<html:radio property="indicadorBloqueio" value="2" />Não</strong></td>
							</tr>
						</logic:equal>

						<logic:equal name="pemissaoIndicadorBloqueio" value="2">
							<tr>
								<td><strong>Indicador de Bloqueio:</strong>
								</td>
								<td><logic:equal name="bloqueio" value="true">
										<input type="radio" name="indicadorBloqueio" value="1"
											disabled checked>
										<strong>Sim</strong>
										<input type="radio" name="indicadorBloqueio" value="2"
											disabled>
										<strong>Não</strong>
									</logic:equal> <logic:equal name="bloqueio" value="false">
										<input type="radio" name="indicadorBloqueio" value="1"
											disabled>
										<strong>Sim</strong>
										<input type="radio" name="indicadorBloqueio" value="2"
					...
					<table height="100%">
						<tr>
							<td width="100%" colspan=2><logic:present name="voltar">
							...
							<td align="right"><gsan:controleAcessoBotao
									name="botaoConcluir" value="Atualizar"
									onclick="javascript:validarForm(document.forms[0]);"
									url="atualizarQuadraAction.do" />
								<!-- 
...
<script language="JavaScript">
	document.forms[0].botaoConcluir.onclick = function() {
