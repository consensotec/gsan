...
<%@ page import="gsan.cadastro.geografico.Bairro" %>
<%@ page import="gsan.cadastro.endereco.Cep" %>
...
function recuperarDadosPopup(codigoRegistro, descricaoRegistro, tipoConsulta) {
    ...
    if (tipoConsulta == 'municipio') {
      ...
      form.action = "exibirAtualizarLogradouroBairroAction.do?removerColecaoBairro=OK&removerColecaoCep=OK";
    ...
    if (tipoConsulta == 'bairro') {
      ...
      //Código responsável pela atualização automática da coleção
      form.action = "exibirAtualizarLogradouroBairroAction.do";
	  submeterFormPadrao(form);
    ...
    if (tipoConsulta == 'cep') {
      ...
      form.action = 'exibirAtualizarLogradouroBairroAction.do?cdCepAtual=' + cdCepAtual + '&cdCepNovo=' + codigoRegistro;
	  submeterFormPadrao(form);
 ...
 function limparPesquisaBairro() {
    ...
    form.action = "exibirAtualizarLogradouroBairroAction.do?removerColecaoBairro=OK";
	submeterFormPadrao(form);
  ...
  function limparPesquisaCep(){
    ...
    form.action = "exibirAtualizarLogradouroBairroAction.do?removerColecaoCep=OK";
	submeterFormPadrao(form);
  }
...
function validarForm(form){
	...
	if(validateLogradouroActionForm(form)){
    	...
    	/*else if (document.getElementById("bairroCarregado") == null){
    		...
    		form.action = 'atualizarLogradouroBairroAction.do?confirmarLogradouroMesmoNome=NAO';
    		submeterFormPadrao(form);
    	}
    }
}
	...
	if (objMunicipio.value.length < 1){
		...
		if (tipoSelecao == "1" && objBairro.value.length > 0){
			...
			if (!testarCampoValorZero(objBairro, "Bairro(s)")){
				...
				form.action = "exibirAtualizarLogradouroBairroAction.do?adicionarBairroColecao=OK";
			submeterFormPadrao(form);
			}
		}
		...
		//CEP
			...
			if (!testarCampoValorZero(objCEP, "CEP(s)")){
				...
				form.action = "exibirAtualizarLogradouroBairroAction.do?adicionarCepColecao=OK";
				submeterFormPadrao(form);
			}
...
function remover(idObj, idLog,tipoRemocao){
	...
	if (confirm('Confirma Remoção?')){
		if (tipoRemocao == "1"){
			form.action = "exibirAtualizarLogradouroBairroAction.do?idBairro=" + idObj +"&idLogradouro="+ idLog ;
		}
		else{
			form.action = "exibirAtualizarLogradouroBairroAction.do?idCep=" + idObj +"&idLogradouro="+ idLog ;
		}
		submeterFormPadrao(form);
	}
}
...
function recuperarDadosQuatroParametros(idRegistro, descricaoRegistro, codigoRegistro, tipoConsulta) {
	...
	if (tipoConsulta == 'bairro') {
      ...
      form.action = 'exibirAtualizarLogradouroBairroAction.do?idBairroAtual=' + idBairroAtual + '&idBairroNovo=' + idRegistro;
	  submeterFormPadrao(form);
	}
...
<html:form action="/atualizarLogradouroBairroAction.do"
			...
			<table width="100%" border="0">
				...
				<tr>
					...
					<td colspan="2"><html:text maxlength="4" property="idMunicipio" size="4"
						tabindex="5"
						onkeypress="return validaEnter(event, 'exibirAtualizarLogradouroBairroAction.do?removerColecaoBairro=OK&removerColecaoCep=OK', 'idMunicipio');return isCampoNumerico(event);"/>
					<a
				...
				<tr>
					...
					<td><html:text maxlength="4" property="codigoBairro" size="4"
						tabindex="6"
						onkeypress="javascript:return validaEnterDependencia(event, 'exibirAtualizarLogradouroBairroAction.do', this,document.forms[0].idMunicipio.value,'Município');return isCampoNumerico(event);" />
				...
				<tr>
					<td colspan="2">
						...
						onClick="javascript:window.location.href='/gsan/exibirAtualizarLogradouroBairroAction.do?menu=sim&idRegistroAtualizacao=${sessionScope.logradouro.id}'" style="width: 80px" />&nbsp;
						<input type="button" name="Button" class="bottonRightCol"
					...
					<gsan:controleAcessoBotao name="Button" value="Atualizar"
							  onclick="javascript:validarForm(document.forms[0]);" url="atualizarLogradouroBairroAction.do"/>
