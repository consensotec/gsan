...
function validarForm(form){
	...
	if(testarCampoValorZero(document.InserirProcessoMensalEventualActionForm.idProcesso, 'Código do Processo') && testarCampoValorZero(document.InserirProcessoMensalEventualActionForm.idProcessoIniciadoPrecedente, 'Código do Processo Iniciado Precedente')){
		...
		if(validateInserirProcessoMensalEventualActionForm(form) && validaTamanhoHoraAgendamento() && validarHoraCompleta(form.horaAgendamento.value)){
...
function validaTamanhoHoraAgendamento(){
	var form = document.forms[0];
	
	if(form.horaAgendamento.value.length < 8){
		alert ("Hora inválida.");
		return false;
	}else{
		return true;
...
<html:form action="/inserirProcessoMensalEventualAction.do"
	...
	type="gsan.gui.batch.InserirProcessoMensalEventualActionForm"
	method="post"
