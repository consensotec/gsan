...
<script language="JavaScript"
  ...
  <script type="text/javascript">
	...
	function clicouAtualizar(id){
		window.location.href = "/gsan/exibirAtualizarContratoParcelamentoClienteAction.do?contratoAtualizarId="+id+"&consultar=sim";
	}
	 
	function cancelarContrato(idContrato, idClienteContrato){
		abrirPopup('exibirCancelarContratoParcelamentoClienteAction.do?idContrato='+idContrato + '&idClienteContrato=' + idClienteContrato, 200, 350);
	}
	...
	function clicouEmitirExtrato(id){
		window.location.href = "/gsan/exibirEmitirExtratoContratoParcelamentoPorClienteAction.do?contratoAtualizarId="+id+"";
	}
...
<html:form action="/atualizarContratoParcelamentoClienteAction.do"
	...
	type="gsan.gui.cobranca.contratoparcelamento.AtualizarContratoParcelamentoPorClienteActionForm"
	method="post">
			               ...
			               <table border="0">    
			                    ...
			                    <tr> 
			                      <td align="left" colspan="3"> 
			                        ...
			                        <input name="btnAtualizar" type="button" onclick="clicouAtualizar('<c:out value="${contratoManter.id}"></c:out>');" class="bottonRightCol"  value="Atualizar" <c:if test="${contratoManter.parcelamentoSituacao.id != 1 || desabilitaAtualizar == true}">disabled="disabled"</c:if> /> 
			                        <input name="btnCancelarContrato" <c:if test="${contratoManter.parcelamentoSituacao.id != 1 }">disabled="disabled"</c:if> type="button" onclick="cancelarContrato('<c:out value="${contratoManter.id}"></c:out>', ${contratoClienteManter.id} );" class="bottonRightCol"  value="Cancelar Contrato" />
			                        <input name="btnEmitirContrato" type="button" onclick="toggleBox('demodiv',1);" class="bottonRightCol"  value="Emitir Contrato" />
			                        <input name="btnEmitirComprovanteContrato" type="button" onclick="javascript:abrirPopup('exibirEmitirComprovantePagamentoContratoParcelamentoClienteAction.do?idContrato=<c:out value="${contratoManter.id}"></c:out>',500,700)" class="bottonRightCol"  value="Emitir Comprovante de Pagamento" />
			                        <input name="btnEmitirExtratoContratoParcelamento" type="button" onclick="clicouEmitirExtrato('<c:out value="${contratoManter.id}"></c:out>');" class="bottonRightCol"  value="Emitir Extrato de Contrato de Parcelamento" />
			                      </td>
