...
<div id="popupContact">
 ...
 function clicouInformar(valorParcelTotal,totalParcelasPopUp){
	 ...
	 var valorContrato = document.getElementById("valorParceladoPopUp").innerHTML;

	 if(valorContrato == null || valorContrato == ""){
		 var valorParcela = document.getElementById("valorDaParcelaAdd");
		 valorContrato = valorParcela;
		 }
	 ...
	 if(parseFloat(valorContrato) != parseFloat(valorParcelTotal)){
		...
		if(totalParcelasPopUp < numeroParcelasPopUp.value){
			 ...
			 form.action = "exibirInserirContratoParcelamentoClienteAction.do?method=informarParcela";
...
function clicouVoltar(){
		 ...
		 if(parcelaInicial.value != null && parcelaInicial.value != ""){
			 ...
			 if(valorParcela.value != null && valorParcela.value != ""){
	      		...
	      		if(parseInt(parcelaFinal.value) < parseInt(parcelaInicial.value)){
		      		...
		      		var form = document.forms[0];
					form.action = "exibirInserirContratoParcelamentoClienteAction.do?method=inserirParcela&parcelaInicial=" + parcelaInicial.value +"&parcelaFinal="+ parcelaFinal.value +"&valorParcela="+valorParcela.value+"&numeroParcelasPopUp="+numeroParcelasPopUp.value;
		            document.forms[0].submit();
	      		}
 ...
 function clicouCancelar(){
	 var form = document.forms[0];
	 form.action = "exibirInserirContratoParcelamentoClienteAction.do?method=cancelarParcela";
     document.forms[0].submit();
 }
 
 function removerParcela(parcelIni,parcelFim, grupoRemover){
	 var form = document.forms[0];
		form.action = "exibirInserirContratoParcelamentoClienteAction.do?method=removerParcela&parcelaInicial=" + parcelIni+"&parcelaFinal="+parcelFim +"&grupoValoresRemover="+grupoRemover;
     document.forms[0].submit();
 }
 </script>
			...
			<td valign="top" >
					...
					<table>
						<tr>
							...
							<td width="100" style="border: 0pt none; background: none repeat scroll 0% 0% rgb(239, 239, 239);">
								<div align="left" style="text-align: right;">
									<fmt:formatNumber value="${valorContaSelecaoTotal}" minFractionDigits="2" maxFractionDigits="2" type="number" />
								 </div>
							</td>
						</tr>
						...
						<tr>
							...
							<td style="border: 0pt none; background: none repeat scroll 0% 0% rgb(239, 239, 239);">
								<div align="left" id="valorParceladoPopUp" style="text-align: right;">
								</div>
							</td>
						</tr>	
					</table>
					<table>
						...
						<tr>
							...
							<td width="700"><strong> 
								<input name="numeroParcelasPopUp" id="numeroParcelasPopUp" type="text" 
									onkeypress="return isCampoNumerico(event);"
									size="4" maxlength="4" value="<c:out value="${numeroParcelasPopUp}"></c:out>"
					...
					<table>
						...
						<tr>
							...
							<td width="700">
								<input name="parcelaInicial" id="parcelaInicial" type="text" size="4" 
									onkeypress="return isCampoNumerico(event);" maxlength="4" 
									style="text-align:right;"/> 
								a 
								<input name="parcelaFinal" id="parcelaFinal" type="text" size="4" maxlength="4" 
									onkeypress="return isCampoNumerico(event);" 
									style="text-align:right;"/>
							</td>
						</tr>
						<tr>
							...
							<td width="700">
								<input name="valorDaParcelaAdd" id="valorDaParcelaAdd" 
									type="text" size="15" maxlength="17" 
									style="text-align:right;"
									onkeypress="return verificaVirgula(event, 'valorDaParcelaAdd');" 
								 	onkeyup="formataValorMonetario(this, 17);" /> 
							</td>
							<td>
								 <input name="addParcela" type="button" 
										...
										if(listaDeParcelasPopUp != null){ %>
											...
											{ 
												...
												for(int j = 0; j < listaDeParcelasPopUp.size(); j++){
													...
													if(listaValoresDistintosPopUp.get(i).floatValue() == listaDeParcelasPopUp.get(j).getValor().floatValue()){
															...
															if (j+1 == listaDeParcelasPopUp.size() || !idsParcelas.contains(listaDeParcelasPopUp.get(j).getId()) ||
																...
																idsParcelas.add(listaDeParcelasPopUp.get(j).getId());
