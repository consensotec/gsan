	...
	<script language="JavaScript">
		    ...
		    form.assunto.value = "";
   		   ...
   		   form.dataVigenciaFinal.value = "";
   		    form.dataVigenciaInicial.value = "";
   		     form.qtdFaturasParceladas.value = "";
		...
		function verificarData(){
			...
			if(form.dataVigenciaFinal.disabled == false){
				...
				abrirCalendario('InserirResolucaoDiretoriaContratoParcelamentoActionForm', 'dataVigenciaFinal');
			}
		}
		...
		function adicionarParcela(){
            ...
            if(form.qtdFaturasParceladas == null || form.qtdFaturasParceladas.value == "" || form.qtdFaturasParceladas.value == "0"){
             	...
             	if(parcelInt > qtdInt){
 					...
 					form.action = "inserirResolucaoDiretoriaContratoParcelamentoAction.do?method=inserirParcela&parcela=" + parcela + "&taxaJuros="+taxaJuros;
		...
		function inserir(){
			...
			if(validar){
				var form = document.forms[0];
				form.action = form.action + "?method=concluirInserirRD";
	            document.forms[0].submit();
			}
		}
		...
		function verifica_data (data) { 
		 ...
		 function selecionouParcelJurosNao(){
			 ...
			 form.action = "inserirResolucaoDiretoriaContratoParcelamentoAction.do?method=limparColecaoParcelas";
	         document.forms[0].submit();
		 }
...
<body leftmargin="5" topmargin="5" onload="javascript: verificaRadioButtons('<c:out value="${contratoCadastrar.indicadorDebitoAcrescimo}"></c:out>','<c:out value="${contratoCadastrar.indicadorParcelamentoJuros}"></c:out>','<c:out value="${contratoCadastrar.indicadorInformarParcela}"></c:out>')">
	...
	<html:form action="/inserirResolucaoDiretoriaContratoParcelamentoAction.do"
		...
		type="gsan.gui.cobranca.contratoparcelamento.InserirResolucaoDiretoriaContratoParcelamentoActionForm"
		method="post">
			...
			<td valign="top" class="centercoltext">
				...
				<table width="100%" border="0" align="center" cellpadding="0"
					...
					<tr>
						...
						<td class="parabg">Inserir Resolu&ccedil;&atilde;o de Diretoria
				...
				<table width="100%" border="0">
					...
					<tr>
						<td width="230"><strong>N&uacute;mero da RD:<font color="#FF0000">*</font></strong></td>
						<td width="383">
							<html:text property="numero" size="15" value="${contratoCadastrar.numero}"
							maxlength="15" />
						</td>
					</tr>
					<tr>
						<td><strong>Assunto da RD:<font color="#FF0000">*</font></strong></td>
						<td><html:text property="assunto" size="50" value="${contratoCadastrar.assunto}"
							maxlength="50" /></td>
					</tr>
					<tr>
						...
						<td align="right">
						<div align="left">
							<html:text value="${contratoCadastrar.dataVigenciaInicioFormatada}"
							onkeyup="mascaraData(this, event);limpaDataFinal();"
							property="dataVigenciaInicial" size="10" maxlength="10" /> <a
							href="javascript:abrirCalendario('InserirResolucaoDiretoriaContratoParcelamentoActionForm', 'dataVigenciaInicial')">
						<img border="0"
							src="<bean:message key='caminho.imagens'/>calendario.gif"
							width="20" border="0" align="middle" alt="Exibir Calendário" /></a>
						a
						&nbsp; <html:text onkeyup="mascaraData(this, event);"
							property="dataVigenciaFinal" size="10" maxlength="10" value="${contratoCadastrar.dataVigenciaFinalFormatada}" /> <a
							href="javascript:abrirCalendario('InserirResolucaoDiretoriaContratoParcelamentoActionForm', 'dataVigenciaFinal')"><img
							border="0"
							src="<bean:message key='caminho.imagens'/>calendario.gif"
							width="20" border="0" align="middle" alt="Exibir Calendário" /></a>
						dd/mm/aaaa
					...
					<tr>
						...
						<div align="left">
							<strong> 
								<html:text  property="qtdFaturasParceladas" size="3"  onkeypress="return isCampoNumerico(event);"
									maxlength="3" value="${contratoCadastrar.qtdFaturasParceladas}"  ></html:text>
							</strong>
						</div>
						</td>
					</tr>
					...
					<tr>
						...
						<table width="580" border="0" bgcolor="#90c7fc" id="tbParcelas">
							...
							<c:forEach items="${parcelas}" var="parcela" varStatus="qtd">
					...
					<tr>
						...
						<td align="right"><input type="button" name="Submit"
							class="bottonRightCol" value="Inserir" onclick="javascript: inserir();"></td>
					</tr>
				</table>
