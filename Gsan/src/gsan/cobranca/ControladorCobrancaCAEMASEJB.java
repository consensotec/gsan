...
import gsan.cobranca.bean.DeterminarValorDescontoAcrescimosImpontualidadeHelper;
import gsan.cobranca.bean.DeterminarValorDescontoPagamentoAVistaHelper;
...
public class ControladorCobrancaCAEMASEJB extends ControladorCobranca implements SessionBean {
	...
	public BigDecimal determinarValorDescontoPagamentoAVista(
		...
		if (valorDescontoPagamentoAVista.getParcelamentoPerfil().getPercentualDescontoPagamentoAVista() != null && 
			...
			valorDescontoAcrescimos.getValorTotalAcrescimosImpontualidadePorAntiguidade().setScale(
			Parcelamento.CASAS_DECIMAIS, Parcelamento.TIPO_ARREDONDAMENTO)
			.add(valorDescontoAcrescimos.getValorAcrescimosImpontualidadeGuiaPagamento()
...
//													.getNumeroSequenciaDocumento()));
							...
							if (cobrancaDocumento.getEmissao() != null) {
								Integer numeroDiasVencimento = acaoCobranca.getNumeroDiasVencimento() != null ? acaoCobranca.getNumeroDiasVencimento() : new Integer(0);
								dataVencimento = Util.formatarData(Util.adicionarNumeroDiasDeUmaData(cobrancaDocumento.getEmissao(), numeroDiasVencimento));
							}
...
//							}
							...
							// [UC0873] GerarTxtAcaoCobranca - Inserido o campo valorDesconto no layout do arquivo de texto
							cobrancaDocumentoTxt.append(Util.completaString(Util.formataBigDecimal(cobrancaDocumento.getValorDesconto(), 2, true), 14));
