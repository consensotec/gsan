...
public class BatchAtualizarPagamentosContasCobrancaMDB implements
	...
	public void onMessage(Message message) {
		if (message instanceof ObjectMessage) {
			...
			try {
				Collection colecaoIdsLocalidade = (Collection) ((Object[]) objectMessage.getObject())[0];
				Iterator iter = colecaoIdsLocalidade.iterator();
				while (iter.hasNext()) {
					Integer idLocalidade = (Integer) iter.next();
					this.getControladorCobranca().
					atualizarPagamentosContasCobranca(idLocalidade,
							                          (Integer) ((Object[]) objectMessage.getObject())[1],
							                          (Integer) ((Object[]) objectMessage.getObject())[2]);
				}
				//[SB0014] verifica se a empresa tem indicador de incluir contas canceladas no pagamento
				this.getControladorCobranca().verificarCancelamentoDeContas();
				
				Collection<Object[]> dadosComandosEncSemPena = this.getControladorCobranca().pesquisarComandosEncerradosSemPenalidades();			
				if(dadosComandosEncSemPena !=null && !dadosComandosEncSemPena.isEmpty()){
					//[UC0867][SB0015]
					this.getControladorCobranca().inserirEmpresaContaCobrancaPena(dadosComandosEncSemPena);
				}
