...
public class RelatorioSaldoContasAReceberContabil extends TarefaRelatorio {
	...
	public Object executar() throws TarefaException {
		...
		RelatorioSaldoContasAReceberContabilBean relatorioUltimo = (RelatorioSaldoContasAReceberContabilBean) colecao[tamanhoColecao-count+1];
		...
		Iterator iColecao = colecaoBean.iterator();
		String impostosDeduzidos = "";
		...
		while (iColecao.hasNext()){
			RelatorioSaldoContasAReceberContabilBean relatorio = (RelatorioSaldoContasAReceberContabilBean) iColecao.next();
			
			if (relatorio.getIdLancamentoTipo() != null &&
					relatorio.getIdLancamentoTipo() == 85 && 
					relatorio.getDescricaoLancamentoItem().equals("IMPOSTOS DEDUZIDOS")){
				
				impostosDeduzidos = relatorio.getDescricaoLancamentoItem();
				valorImpostosDeduzidosParticular = valorImpostosDeduzidosParticular.add(relatorio.getValorImpostosDeduzidosParticular()) ;
				valorImpostosDeduzidosPublico = valorImpostosDeduzidosPublico.add(relatorio.getValorImpostosDeduzidosPublico());
				valorImpostosDeduzidosTotal = relatorio.getValorImpostosDeduzidosTotal();
			}
		}
		
		if (!impostosDeduzidos.equals("")){
			
			parametros.put("impostosDeduzidos", impostosDeduzidos);
			parametros.put("valorImpostosDeduzidosParticular", valorImpostosDeduzidosParticular);
			parametros.put("valorImpostosDeduzidosPublico", valorImpostosDeduzidosPublico);
			parametros.put("valorImpostosDeduzidosTotal", valorImpostosDeduzidosTotal);
		}
