...
public class ExibirAtualizarContratoAdesaoAction  extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		// Seta o retorno
		ActionForward retorno = actionMapping.findForward("atualizarContratoAdesao");
		...
		if(Util.verificarNaoVazio(primeiraVez)){
			...
			filtro.adicionarParametro(new ParametroSimples(FiltroContratoAdesao.ID, idRegistroAtualizar));					
			Collection colecaoContratoAdesao = fachada.pesquisar(filtro, ContratoAdesao.class.getName());
			if (colecaoContratoAdesao != null && !colecaoContratoAdesao.isEmpty()) {
				contratoAdesao = (ContratoAdesao) Util.retonarObjetoDeColecao(colecaoContratoAdesao);
			}
			sessao.setAttribute("contratoAdesao", contratoAdesao);
		}else{
			contratoAdesao = (ContratoAdesao) sessao.getAttribute("contratoAdesao");
		}

		Integer id = contratoAdesao.getId();
		if (id != null) {
		...
		String descricao = contratoAdesao.getDescricao();
		if (descricao != null) {
		...
		String descricaoAbreviada = contratoAdesao.getDescricaoAbreviada();
		if (descricaoAbreviada != null) {
			form.setDescricaoAbreviada(String.valueOf(descricaoAbreviada));
		} else {
			form.setDescricaoAbreviada("");
		}

		Short indicadorUso = contratoAdesao.getIndicadorUso();
