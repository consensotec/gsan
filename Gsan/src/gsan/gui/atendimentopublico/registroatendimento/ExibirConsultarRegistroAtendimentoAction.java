...
import gsan.atendimentopublico.registroatendimento.FiltroRegistroAtendimentoConta;
import gsan.atendimentopublico.registroatendimento.FiltroRegistroAtendimentoPagamentoDuplicidade;
...
import gsan.atendimentopublico.registroatendimento.FiltroSolicitanteFone;
...
public class ExibirConsultarRegistroAtendimentoAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if (registroAtendimento.getNnCoordenadaNorte() != null) {
			if(registroAtendimento.getNnCoordenadaNorte().compareTo(BigDecimal.ZERO) == 0){
		...
		if (registroAtendimento.getNnCoordenadaLeste() != null) {
			if(registroAtendimento.getNnCoordenadaLeste().compareTo(BigDecimal.ZERO) == 0){
		...
		if (colecaoRAContas != null && !colecaoRAContas.isEmpty()) {
			...
			Iterator it = colecaoRAContas.iterator();
			
			while(it.hasNext()){
				
				RegistroAtendimentoConta ra = (RegistroAtendimentoConta) it.next();

				if (ra != null && ra.getContaGeral() != null && 
						ra.getContaGeral().getIndicadorHistorico() == ConstantesSistema.NAO){
					
					FiltroConta filtroConta =  new FiltroConta();
					filtroConta.adicionarParametro(new ParametroSimples(FiltroConta.ID, ra.getContaGeral().getId()));
					...
					ra.getContaGeral().setConta(conta);
				}
				else{
					if (ra != null && ra.getContaGeral() != null){
						FiltroContaHistorico filtroContaHistorico =  new FiltroContaHistorico();
						filtroContaHistorico.adicionarParametro(new ParametroSimples(FiltroContaHistorico.ID, ra.getContaGeral().getId()));
						...
						ra.getContaGeral().setContaHistorico(contaHistorico);
					}
				}
			}
