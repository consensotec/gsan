...
public class ConsultarOrdemServicoDoArquivoTextoAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		ConsultarOrdemServicoDoArquivoTextoActionForm form = (ConsultarOrdemServicoDoArquivoTextoActionForm) actionForm;
		...
		if (idsOrdensServico != null && idsOrdensServico.length > 0) {
			...
			for (int i = 0; i < idsOrdensServico.length; i++) {
				...
				if (dadosOS[2] != null
	        			&& ((Short)dadosOS[2]).compareTo(ConstantesSistema.NAO) == 0) {
		...
		fachada.inserirProcessoIniciadoParametrosLivres(parametros, 
          		Processo.PROCESSAR_ATUAL_ORDENS_SERVICO_ARQ_TXT, usuarioLogado);
