...
public class ConsultarArquivoTextoOSVisitaAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		ActionForward retorno = actionMapping
				.findForward("consultarDadosOSVisita");
		...
		String idLeiturista = form.getLeituristaID();
		String idSituacaoArquivo = form.getSituacaoArquivoTexto();
		
		
		//Fazendo a busca nos arquivos de visita
		Collection<ArquivoTxtOSVisitaHelper> colecaoArquivoTextoOSVisita = 
					fachada.buscarColecaoArquivoTextoOSVisita(
															idLocalidade,
		...
		if(colecaoArquivoTextoOSVisita.size() > 0)
			sessao.setAttribute("colecaoArquivoTextoOSVisita", colecaoArquivoTextoOSVisita);
		else
			throw new ActionServletException("atencao.arquivo_texto_visita_inexistente");
