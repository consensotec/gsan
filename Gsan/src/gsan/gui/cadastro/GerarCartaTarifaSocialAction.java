 ...
 * @created 01/04/2011
 */
public class GerarCartaTarifaSocialAction extends GcomAction {
	/**
	 * < <Descrição do método>>
	 * 
	 * @param actionMapping
	...
	public ActionForward execute(ActionMapping actionMapping,
			...
			while (itcolacaoTarifaSocialComandoCartaSessao.hasNext()){
				...
				for(int x=0; x<idsComandosArray.length; x++){
					...
					if (comando.getId().equals(new Integer(idsComandosArray[x]))){
						...
						comando.setDataGeracao(new Date());
						fachada.atualizar(comando);
		...
		Fachada.getInstancia().inserirProcessoIniciadoParametrosLivresAguardandoAutorizacao(parametros, 
