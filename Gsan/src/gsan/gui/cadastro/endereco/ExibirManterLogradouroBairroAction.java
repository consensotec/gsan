...
import gsan.cadastro.endereco.FiltroOSProgramaCalibragem;
...
import gsan.cadastro.endereco.bean.ManterLogradouroBairroHelper;
import gsan.cadastro.endereco.bean.ManterLogradouroHelper;
 ...
 * @date   28/06/2006 
 */
public class ExibirManterLogradouroBairroAction extends GcomAction {
	/**
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		ActionForward retorno = actionMapping.findForward("manterLogradouroBairro");
		...
		sessao.setAttribute("indicadorImportanciaLogradouro","true");
		...
		if (httpServletRequest.getAttribute("filtroLogradouro") != null) {
			...
			if (fachada.registroMaximo(LogradouroCep.class) > ConstantesSistema.NUMERO_MAXIMO_REGISTROS_MANUTENCAO) {
				...
				// é chamada
				retorno = actionMapping.findForward("filtrarLogradouroBairro");
		...
		if (retorno.getName().equalsIgnoreCase("manterLogradouroBairro")) {

			// 1º Passo - Pegar o total de registros através de um count da
			// consulta que aparecerá na tela
			
			Integer totalRegistros = fachada.pesquisarLogradouroBairroCompletoCount(
			...
			Collection<LogradouroBairro> colecaoLogradouro = fachada.pesquisarLogradouroBairroCompleto(idMunicipio, idBairro, nomeLogradouro,
					nomePopularLogradouro, idTipoLogradouro,
					idTituloLogradouro, codigoCep, idLogradouro, indicadorUso, tipoPesquisa, tipoPesquisaPopular,
			...
			Collection<ManterLogradouroBairroHelper> colecaoParaExibicao = new ArrayList<ManterLogradouroBairroHelper>();
			
			for (LogradouroBairro logradouro : colecaoLogradouro){
				
				ManterLogradouroBairroHelper helper = new ManterLogradouroBairroHelper();
				
				helper.setLogradouro(logradouro);
					
				helper.setBairro(logradouro.getBairro().getNome());
				
				if(logradouro.getProgramaCalibragem() != null){
				
					helper.setGrauImportancia(logradouro.getProgramaCalibragem().getGrauImportancia());
				} 
			...
			if (atualizar != null && colecaoLogradouro != null
				...
				retorno = actionMapping.findForward("atualizarLogradouroBairro");
				LogradouroBairro logradouroBairro = (LogradouroBairro) colecaoLogradouro.iterator()
						.next();
				httpServletRequest.setAttribute("idRegistroAtualizacao",
						logradouroBairro.getId());
			}
		}
		return retorno;
	}
