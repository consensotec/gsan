...
public class ExibirFiltrarQuadraAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		ActionForward retorno = actionMapping.findForward("filtrarQuadra");
		...
		if (idRota != null && !idRota.trim().equals("")) {
			...
			Collection colecaoRotas = fachada.pesquisar(filtroRota,
					Rota.class.getName());
			...
			if (colecaoRotas != null && !colecaoRotas.isEmpty()) {
				...
				if (rota.getIndicadorRotaAlternativa().shortValue() == ConstantesSistema.SIM) {
					...
					filtrarQuadraActionForm.setCodigoRota(rota.getCodigo()
							.toString());
				}
		...
		if (objetoConsulta != null
			...
			setorComercialCD = filtrarQuadraActionForm.getSetorComercialCD();

			switch (Integer.parseInt(objetoConsulta)) {
			// Localidade
			case 1:

				pesquisarLocalidade(filtrarQuadraActionForm, fachada,
						httpServletRequest);
			...
			case 2:
				...
				pesquisarSetorComercial(filtrarQuadraActionForm, fachada,
						httpServletRequest);
		...
		if (httpServletRequest.getParameter("desfazer") != null
			...
			filtrarQuadraActionForm.setIndicadorAtualizacaoCadastral("3");
		...
		if (sessao.getAttribute("voltar") != null
			...
			if (sessao.getAttribute("indicadorUso") != null) {
				filtrarQuadraActionForm.setIndicadorUso(sessao.getAttribute(
						"indicadorUso").toString());
				// filtrarQuadraActionForm.setTipoPesquisa(ConstantesSistema.TIPO_PESQUISA_INICIAL.toString());
			}
			if (sessao.getAttribute("IndicadorAtualizacaoCadastral") != null) {
				filtrarQuadraActionForm.setIndicadorAtualizacaoCadastral(sessao.getAttribute(
						"indicadorAtualizacaoCadastral").toString());
			}
		}
		sessao.removeAttribute("idRegistroAtualizacao");
		sessao.removeAttribute("indicadorUso");
		sessao.removeAttribute("indicadorAtualizacaoCadastral");
	...
	private void pesquisarLocalidade(
			FiltrarQuadraActionForm filtrarQuadraActionForm, Fachada fachada,
		...
		if (localidadeID == null || localidadeID.trim().equalsIgnoreCase("")) {
			...
			filtrarQuadraActionForm.setLocalidadeNome("Informe Localidade");
			...
			if (colecaoPesquisa == null || colecaoPesquisa.isEmpty()) {
				...
				filtrarQuadraActionForm.setLocalidadeNome(objetoLocalidade
						.getDescricao());
				httpServletRequest.setAttribute("corLocalidade", "valor");

				httpServletRequest
						.setAttribute("nomeCampo", "setorComercialCD");
			}
		}
	}
