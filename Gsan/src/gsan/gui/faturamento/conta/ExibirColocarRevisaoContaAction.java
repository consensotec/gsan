...
import gsan.faturamento.conta.FiltroMotivoRevisaoConta;
...
public class ExibirColocarRevisaoContaAction extends GcomAction {
    ...
    public ActionForward execute(ActionMapping actionMapping,
        ...
        if (sessao.getAttribute("colecaoMotivoRevisaoConta") == null){
        	...
        	filtroMotivoRevisaoConta.adicionarCaminhoParaCarregamentoEntidade(
        			FiltroMotivoRevisaoConta.PERMISSAO_ESPECIAL);
        	...
        	Collection<Integer> colecaoIdsContaMotivoRevisaoPermissaoEspecial = new ArrayList<Integer>();
        	
        	Iterator<?> it = colecaoMotivoRevisaoConta.iterator();
        	while(it.hasNext()){
        		ContaMotivoRevisao contaMotivoRevisao = (ContaMotivoRevisao) it.next();
        		
        		if(contaMotivoRevisao.getPermissaoEspecial() != null){
        			boolean temPermissaoContaMotivoRevisao = 
        				fachada.verificarPermissaoEspecial(contaMotivoRevisao.getPermissaoEspecial().getId(), usuarioLogado);
        			
        			if(!temPermissaoContaMotivoRevisao){
        				colecaoIdsContaMotivoRevisaoPermissaoEspecial.add(contaMotivoRevisao.getId());
        			}
        		}
        	}
        	
        	if(!Util.isVazioOrNulo(colecaoIdsContaMotivoRevisaoPermissaoEspecial)){
        		for(Integer idContaMotivoRevisao : colecaoIdsContaMotivoRevisaoPermissaoEspecial){
        			it = colecaoMotivoRevisaoConta.iterator();
                	while(it.hasNext()){
                		ContaMotivoRevisao contaMotivoRevisao = (ContaMotivoRevisao) it.next();
                		
                		if(idContaMotivoRevisao.equals(contaMotivoRevisao.getId())){
                			colecaoMotivoRevisaoConta.remove(contaMotivoRevisao);
                			break;
                		}
                	}
        		}
