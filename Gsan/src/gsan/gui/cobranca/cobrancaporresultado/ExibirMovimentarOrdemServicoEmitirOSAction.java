...
public class ExibirMovimentarOrdemServicoEmitirOSAction extends GcomAction {
    ...
    public ActionForward execute(ActionMapping actionMapping,
		...
		if (httpServletRequest.getParameter("pesquisarQtdContas") != null
			...
			if (movimentarOrdemServicoEmitirOSHelper != null) {
				...
				if (agruparPorImovel) {
					Collection<Object[]> colecaoDados = this.getFachada()
		...
		//2ª div
		if (httpServletRequest.getParameter("pesquisarOSEmpresaCobranca") != null
				&& httpServletRequest.getParameter("pesquisarOSEmpresaCobranca").equalsIgnoreCase("SIM")) {
			...
			Collection<Object[]> colecaoDados = Fachada.getInstancia().
				pesquisarDadosOSGeradasPelaEmpresa(new Integer(form.getIdComandoContaCobranca()), 
						idTipoServico);
		...
		if (httpServletRequest.getParameter("pesquisarOSRA") != null
			...
			Collection<Object[]> colecaoDados = Fachada.getInstancia().
				pesquisarDadosOSRegistroAtendimento(new Integer(form.getIdComandoContaCobranca()), 
						idTipoServico);
	...
	private void pesquisarCamposEnter(
		...
		String idLocalidadeOrigem = form
			...
			if (colecaoLocalidade != null && !colecaoLocalidade.isEmpty()) {
				...
				if (codigoSetorComercialOrigem != null
					...
					if (colecaoSetorComercial != null
						...
						String codigoQuadraInicial = form
						...
						sessao.removeAttribute(
								"setorComercialOrigemInexistente");
					} else {
						form
							.setIdSetorComercialOrigem("");
						form
						...
						form
							.setCodigoSetorComercialDestino("");
						form
								.setDescricaoSetorComercialDestino("");
				...
				sessao.setAttribute("localidadeOrigemInexistente",
						true);
				httpServletRequest.setAttribute("nomeCampo",
						"idLocalidadeOrigem");
			}
		...
		String idLocalidadeDestino = form
			...
			if (colecaoLocalidade != null && !colecaoLocalidade.isEmpty()) {
				...
				if (codigoSetorComercialDestino != null
					...
					if (colecaoSetorComercial != null
						...
						sessao.removeAttribute(
								"setorComercialDestinoInexistente");
					} else {
						form
							.setIdSetorComercialDestino("");
						form
				...
				sessao.removeAttribute(
						"localidadeDestinoInexistente");
			} else {
				form.setIdLocalidadeDestino("");
				form
						.setNomeLocalidadeDestino("LOCALIDADE INEXISTENTE");

				sessao.setAttribute("localidadeDestinoInexistente",
						true);
				httpServletRequest.setAttribute("nomeCampo",
						"idLocalidadeDestino");
			}
	...
	private MovimentarOrdemServicoEmitirOSHelper retornaQtdContas(MovimentarOrdemServicoActionForm form) {
    	...
    	if (colecaoComando != null || !colecaoComando.isEmpty()) {
    		comandoEmpresaCobrancaConta = (ComandoEmpresaCobrancaConta) Util.retonarObjetoDeColecao(colecaoComando);
    	}
    	
		comandoEmpresaCobrancaConta.setId(new Integer(form.getIdComandoContaCobranca()));
		...
		// Intervalo de OS
		if (form.getNumeroOSInicial() != null
				&& !form.getNumeroOSInicial().equals("")
				&& form.getNumeroOSFinal() != null
				&& !form.getNumeroOSFinal().equals("")) {
		...
		// Localidade Inicial
		if (form.getIdLocalidadeOrigem() != null && 
			!form.getIdLocalidadeOrigem().equals("")) {
			...
			if (colecaoLocalidade != null && !colecaoLocalidade.isEmpty()) {
				...
				localidadeInicial.setId(
					new Integer(form.getIdLocalidadeOrigem()));
		...
		// Localidade Final
		if (form.getIdLocalidadeDestino() != null && 
			!form.getIdLocalidadeDestino().equals("")) {
			...
			if (colecaoLocalidade != null && !colecaoLocalidade.isEmpty()) {
				...
				localidadeFinal.setId(
					new Integer(form.getIdLocalidadeDestino()));
		...
		if (form.getCodigoSetorComercialOrigem() != null
				&& !form.getCodigoSetorComercialOrigem().equals("")) {
			...
			filtroSetorComercial.adicionarParametro(new ParametroSimples(
					FiltroSetorComercial.CODIGO_SETOR_COMERCIAL,
					form.getCodigoSetorComercialOrigem()));
		...
		if (form.getCodigoSetorComercialDestino() != null
				&& !form.getCodigoSetorComercialDestino().equals("")) {
		...
		// Quadra Inicial
		if (form.getCodigoQuadraInicial() != null
				&& !form.getCodigoQuadraInicial().equals("")) {
			...
			filtroQuadra.adicionarParametro(new ParametroSimples(
							FiltroQuadra.ID_SETORCOMERCIAL,
							form.getIdSetorComercialOrigem()));

			filtroQuadra.adicionarParametro(new ParametroSimples(
					FiltroQuadra.NUMERO_QUADRA,
					form.getCodigoQuadraInicial()));
		...
		// Unidade Negocio
		if (form.getIdsUnidadeNegocio() != null
				&& form.getIdsUnidadeNegocio().length > 0) {
		...
		// Gerência Regional
		if (form.getIdsGerenciaRegional() != null
				&& form.getIdsGerenciaRegional().length > 0) {
		...
		if (form.getValorMinimo() != null
			...
			comandoEmpresaCobrancaConta.setValorMinimoConta(Util
							.formatarMoedaRealparaBigDecimal(form.getValorMinimo()));
		}
		...
		if (form.getValorMaximo() != null
			...
			comandoEmpresaCobrancaConta.setValorMaximoConta(Util
							.formatarMoedaRealparaBigDecimal(form.getValorMaximo()));
		}
		
		// Valor Débito Inicial
		if(form.getValorMinimoDebito() !=null && 
		   !form.getValorMinimoDebito().equals("")){
			
			algumParametroInformado = true;
			
			comandoEmpresaCobrancaConta
			.setValorMinimoDebito(Util.formatarMoedaRealparaBigDecimal(form.getValorMinimoDebito()));
		}
				
		// Valor Débito Final
		if(form.getValorMaximoDebito() !=null && 
		   !form.getValorMaximoDebito().equals("")){
					
			algumParametroInformado = true;
			
			comandoEmpresaCobrancaConta
			.setValorMaximoDebito(Util.formatarMoedaRealparaBigDecimal(form.getValorMaximoDebito()));
		}
		
		//[FS0004] Verificar valor final menor que valor inicial
		if(sistemaParametro.getIndicadorTotalDebito() == 1){
		...
		if (algumParametroInformado) {
			...
			movimentarOrdemServicoEmitirOSHelper.setComandoEmpresaCobrancaConta(comandoEmpresaCobrancaConta);
			...
			if(sistemaParametro.getIndicadorTotalDebito() == 1){
				movimentarOrdemServicoEmitirOSHelper.setIndicadorTotalDebito(true);
			}
			else{
				movimentarOrdemServicoEmitirOSHelper.setIndicadorTotalDebito(false);
			}
