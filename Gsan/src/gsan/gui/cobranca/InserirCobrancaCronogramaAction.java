					...
					while (iteratorAtividadesCobranca.hasNext()) {
						...
						if (dataPrevista.trim().equals("") && cobrancaAtividade.getIndicadorObrigatoriedade() == 1) {
							...
							if (!dataPrevista.trim().equals("")) {
								...
								/**
								 * Caso a data informada tenha o mês/ano informado igual ao mês/ano informado da 
								 * primeira atividade de outro cronograma do mesmo grupo (mês/ano informado igual a
								 * mês/ano de CAAC_DTPREVISTA da tabela COBRANCA_ACAO_ATIVIDADE_CRONOG com 
									...
									if(!Util.isVazioOrNulo(colecaoDataPrevista)){
										...
										while(it.hasNext()){
											...
											if(Util.compararAnoMesReferencia(anoMesPrevisto, anoMesPrevistoComparar, "=")){
												throw new ActionServletException("atencao.data_prevista_já_inserida_no_cronograma", null, 
