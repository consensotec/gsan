...
import gsan.relatorio.cobranca.RelatorioComandoDocumentoCobranca;
import gsan.relatorio.cobranca.RelatorioDocumentoCobrancaOrdemCorte;
import gsan.relatorio.cobranca.RelatorioDocumentoCobrancaOrdemFiscalizacao;
import gsan.relatorio.cobranca.RelatorioNotificacaoDebito;
import gsan.relatorio.cobranca.RelatorioVisitaCobranca;
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if (documentoTipo != null && (documentoTipo.getId().intValue() == DocumentoTipo.AVISO_CORTE
			...
			relatorio = new RelatorioNotificacaoDebito(
				...
				&& documentoTipo != null
				&& documentoTipo.getId().intValue() == DocumentoTipo.VISITA_COBRANCA) {
			
			relatorio = new RelatorioVisitaCobranca(
					(Usuario) (httpServletRequest.getSession(false))
							.getAttribute("usuarioLogado"));
					...
					SistemaParametro.EMPRESA_CAERN)
				&& documentoTipo != null
				&& documentoTipo.getId().intValue() == DocumentoTipo.ORDEM_CORTE) {
			
			relatorio = new RelatorioDocumentoCobrancaOrdemCorte(
					(Usuario) (httpServletRequest.getSession(false))
							.getAttribute("usuarioLogado"));
			
		}  else if (documentoTipo != null
				&& documentoTipo.getId().intValue() == DocumentoTipo.ORDEM_FISCALIZACAO) {
			
			relatorio = new RelatorioDocumentoCobrancaOrdemFiscalizacao(
					(Usuario) (httpServletRequest.getSession(false))
							.getAttribute("usuarioLogado"));
