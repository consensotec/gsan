...
public class ExibirManterComandoNegativacaoAction extends GcomAction {
    ...
    public ActionForward execute(ActionMapping actionMapping,
		...
		if ((retorno != null)&&(retorno.getName().equalsIgnoreCase("manterComandoNegativacao"))) {
			...
			if (collectionComandoNegativacao.size() == 1 && Util.verificarNaoVazio(checkAtualizar)) {
				...
				if(ConstantesSistema.TIPO_COMANDO_POR_GUIA_PAGAMENTO.equals(helper.getTipoComandoCriterio())) {
					retorno = actionMapping.findForward("atualizarComandoNegativadorPorGuiaPagamento");
				} else {
