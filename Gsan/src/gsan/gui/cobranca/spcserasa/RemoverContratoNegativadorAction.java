 ...
 * @created 20/12/2007
 */
public class RemoverContratoNegativadorAction extends GcomAction {
	/**
	 * @author Yara Taciane de Souza
	 * @date 20/12/2007
	 * 
	...
	public ActionForward execute(ActionMapping actionMapping,
        ...
        try {
        	 fachada.remover(ids, NegativadorContrato.class.getName(), operacaoEfetuada, colecaoUsuarios);
        } catch (Exception e) {
			e.getCause();
		...
		if (retorno.getName().equalsIgnoreCase("telaSucesso")) {
			montarPaginaSucesso(httpServletRequest,
					 ids.length +
					" Contrato(os) de Negativador(es) removido(s) com sucesso.",
					"Realizar outra manutenção de Contrato de Negativador",
