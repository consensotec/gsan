 ...
 * 
 * Esta classe tem por finalidade concluir as informações das duas abas do
 * processo de atualização de um comando de negativação por guia de pagamento
 * 
 * @author André Miranda
 * @date 23/03/2015
 */
public class AtualizarComandoNegativacaoPorGuiaPagamentoAction extends GcomAction {
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
        ...
        comando.setId(Integer.valueOf(form.getIdNegativacaoComando()));
        ...
        criterio.setId(Integer.valueOf(form.getIdNegativacaoCriterio()));
        criterio.setNegativacaoComando(comando);
	...
	private void validarDadosCliente(AtualizarComandoNegativacaoPorCriterioActionForm form, Fachada fachada) {
		...
		if (Util.verificarNaoVazio(referenciaInicial) && Util.verificarNaoVazio(referenciaFinal)) {
	  	...
	  	Integer referenciaMinima = Util.subtrairAnoAnoMesReferencia(getSistemaParametro().getAnoMesArrecadacao(), 5);	  			
