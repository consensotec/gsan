...
public class GerarArquivoOrcamentoSinpAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		this.getFachada().existeDadosUnResumoParcialParaOrcamentoSINP(anoMes);
		
		String indicadorArquivoARSBAN = form.getIndicadorArquivoARSBAN();
		
		if(indicadorArquivoARSBAN.equals(ConstantesSistema.SIM.toString())){

			//Gera do Arquivo texto
			this.getFachada().gerarArquivoTextoOrcamentoSinp(anoMes.intValue());
			...
			int tipoRelatorio = TarefaRelatorio.TIPO_HTML;
	 		...
	 		File arquivoTexto = 
	 			this.getFachada().gerarArquivoTextoPlanejamentoSinp(anoMes.intValue());
		...
		montarPaginaSucesso(
			httpServletRequest, 
			"Arquivo de Texto para Orçamento e SINP gerado com sucesso.",
