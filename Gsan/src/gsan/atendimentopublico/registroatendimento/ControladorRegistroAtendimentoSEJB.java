...
import gsan.atendimentopublico.FiltroEspecificacaoPavimentacaoServicoTipo;
import gsan.atendimentopublico.bean.DadosRAReiteracaoHelper;
...
import gsan.atendimentopublico.ordemservico.EspecificacaoServicoTipoPK;
import gsan.atendimentopublico.ordemservico.FiltroEspecificacaoServicoTipo;
...
import gsan.atendimentopublico.registroatendimento.bean.ExibirRAFaltaAguaImovelHelper;
...
import gsan.atendimentopublico.registroatendimento.bean.GerarNumeracaoRAManualHelper;
...
import gsan.atendimentopublico.registroatendimento.bean.ObterIndicadorExistenciaHidrometroHelper;
import gsan.atendimentopublico.registroatendimento.bean.ObterRAAssociadoHelper;
import gsan.atendimentopublico.registroatendimento.bean.RAFaltaAguaPendenteHelper;
...
import gsan.atendimentopublico.registroatendimento.bean.RegistroAtendimentoPendenteLocalOcorrenciaHelper;
import gsan.atendimentopublico.registroatendimento.bean.ResumoRegistroAtendimentoHelper;
import gsan.atendimentopublico.registroatendimento.bean.ResumoRegistroAtendimentoPorAnoHelper;
...
import gsan.gui.portal.caema.ConsultarTramiteHelper;
...
import gsan.operacional.abastecimento.FiltroAbastecimentoProgramacao;
import gsan.operacional.abastecimento.FiltroManutencaoProgramacao;
...
//			getControladorFaturamento().inserirCreditoRealizadoCategoria(creditoRealizadoInserir,colecaoCategoria);
	 ...
	 * @date 27/03/2015	
	 */
	public Collection<ConsultarTramiteHelper> pesquisarDadosUnidadeNegocio(String idRegistroAtendimento) 
			throws ControladorException{
				
		try {			
				Iterator<Object[]> iterator = repositorioRegistroAtendimento.pesquisarDadosUnidadeNegocio(idRegistroAtendimento).iterator();
			
			Collection <ConsultarTramiteHelper> colecaoTramite = new ArrayList();
			
			while(iterator.hasNext()){ 
				
			
				Object[] unidadeNegocio = iterator.next();					
					
				ConsultarTramiteHelper tramite  = new ConsultarTramiteHelper();
				
				
				if(unidadeNegocio[0] != null){
				
					String data = unidadeNegocio[0].toString().replace("-", "");
					...
					String horaFormatada = data.substring(9, 17);
					
					
					tramite.setDataHoraTramite(dataFormatada +" "+ horaFormatada );
					
				}
				
				if(unidadeNegocio[1] != null){
										
					tramite.setUnidadeDestino((String)unidadeNegocio[1]);
				}
				
				colecaoTramite.add(tramite);
