		...
		if (tipoConsulta.equalsIgnoreCase("conta")) {
			...
			if(idConta != null && !idConta.equals("")){
				...
				if(Util.isVazioOrNulo(colPagamentos)){
					...
					sessao.setAttribute("contaClientes",fachada.pesquisarClientesConta(
							new Integer(idConta.trim())));
					
					sessao.setAttribute("contaClientesAnterior",fachada.pesquisarClientesContaAnterior(
						new Integer(idConta.trim())));
				}else{
