...
public class EmissaoExtratoDebitoAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
        ...
        if(guias != null){
			...
			if ( form.getIndicadorIncluirAcrescimosImpontualidade() != null && 
					( form.getIndicadorIncluirAcrescimosImpontualidade().equals(CobrancaDocumento.INCLUIR_ACRESCIMOS) || 
						form.getIndicadorIncluirAcrescimosImpontualidade().equals(CobrancaDocumento.INCLUIR_ACRESCIMOS_COM_DESCONTO)) ) {
				
				valorAcrescimosImpontualidade = valorAcrescimosImpontualidade.add((BigDecimal)guias[2]).add((BigDecimal)guias[4]).add((BigDecimal)guias[3]);
			...
			} else if ( form.getIndicadorIncluirAcrescimosImpontualidade() != null && 
					form.getIndicadorIncluirAcrescimosImpontualidade().equals(CobrancaDocumento.NAO_INCLUIR_ACRESCIMOS)) {
				// cria os itens de cobran?a de documento para as guias de pagamento
				for (GuiaPagamentoValoresHelper guiaPagamentoValorHelper : colecaoGuiasPagamento) {
					...
					if(qtdAntecipacaoParcelas != null && !qtdAntecipacaoParcelas.equals("")){
						...
						if (debitoCreditoParcelamentoHelper.getColecaoDebitoACobrarParcelamento() != null &&
						   ...
						   while (iterDebito.hasNext()) {
							   ...
							   if (debitoACobrar.getDebitoACobrarGeralOrigem() == null){
