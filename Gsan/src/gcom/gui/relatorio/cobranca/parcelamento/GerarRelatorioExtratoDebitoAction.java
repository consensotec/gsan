...
//				valor = valor.replaceAll("[.]","").replaceAll("[,]", "");
				valorAcrescimosImpontualidade = Util.formatarMoedaRealparaBigDecimal(valor);	
			}
...
//			colecaoDebitosACobrar = obterColecaoDebitosACobrarSemJurosParcelamento(colecaoDebitosACobrar);
			...
			imovel = fachada.pesquisarImovel(idImovel);
			valorDescontoCredito = Util.formatarMoedaRealparaBigDecimal((String)sessao.getAttribute("valorCreditoARealizar").toString());
			valorDocumento = (BigDecimal)sessao.getAttribute("valorPagamentoAVista");
			...
			if(emitirExtratoDebitoCliente != null && emitirExtratoDebitoCliente.equalsIgnoreCase("1")){
		    	...
		    	if (contasGuiasSelecionadas != null) {
			    	...
			    	if(obterOpcoesDeParcelamentoHelper != null){
						...
						valorDocumento = valorTotalComAcrescimo;
					}	
...
//			colecaoDebitosACobrar = obterColecaoDebitosACobrarSemJurosParcelamento(colecaoDebitosACobrar);
		...
		if(idParcelamento != null){
			...
			maiorDataVencimentoContas = null;
			...
			if(indicadorContasRevisao.equals(ConstantesSistema.NAO)){
				...
				BigDecimal valorTotalAcrescimosContasRevisao = (BigDecimal) mapContas.get("valorTotalAcrescimosContasRevisao");
				valorDocumento = valorDocumento.subtract(valorTotalContasRevisao);
				valorDocumento = valorDocumento.subtract(valorTotalAcrescimosContasRevisao);
				valorAcrescimosImpontualidade = valorAcrescimosImpontualidade.subtract(valorTotalAcrescimosContasRevisao);
			...
			BigDecimal valorTotalAcrescimosContasProgramaEspecial = (BigDecimal) mapContas.get("valorTotalAcrescimosContasProgramaEspecial");
			
			valorDocumento = valorDocumento.subtract(valorTotalContasProgramaEspecial);
			
			valorDocumento = valorDocumento.subtract(valorTotalAcrescimosContasProgramaEspecial);
			valorAcrescimosImpontualidade = valorAcrescimosImpontualidade.subtract(valorTotalAcrescimosContasProgramaEspecial);
...
//		 relatorioExtratoDebito.addParametro("nomeDocumento", nomeDocumento);
	...
	private Map retirarContasEmRevisaoDeColecaoContas(Collection<ContaValoresHelper> colecaoContas){
		...
		BigDecimal valorTotalAcrescimosContasRevisao =  BigDecimal.ZERO;
			...
			while (iter.hasNext()) {
				...
				if (conta.getContaMotivoRevisao() != null){
					...
					if (contaValoresHelper.getValorAtualizacaoMonetaria() != null) {
						valorTotalAcrescimosContasRevisao = valorTotalAcrescimosContasRevisao.add(contaValoresHelper.getValorAtualizacaoMonetaria()); 
					}
					
					if (contaValoresHelper.getValorJurosMora() != null) {
						valorTotalAcrescimosContasRevisao = valorTotalAcrescimosContasRevisao.add(contaValoresHelper.getValorJurosMora()); 
					}
					
					if (contaValoresHelper.getValorMulta() != null) {
						valorTotalAcrescimosContasRevisao = valorTotalAcrescimosContasRevisao.add(contaValoresHelper.getValorMulta()); 
					}
		...
		retorno.put("valorTotalAcrescimosContasRevisao",valorTotalAcrescimosContasRevisao);
	...
	private Map retirarContasEmProgramaEspecialDeColecaoContas(Collection<ContaValoresHelper> colecaoContas){
		...
		BigDecimal valorTotalAcrescimosContasProgramaEspecial =  BigDecimal.ZERO;
			...
			while (iter.hasNext()) {
				...
				if(sistemaParametro.getPerfilProgramaEspecial() != null 
					...
					if(conta.getImovelPerfil().getId().equals(sistemaParametro.getPerfilProgramaEspecial().getId())){
						...
						if (contaValoresHelper.getValorAtualizacaoMonetaria() != null) {
							valorTotalAcrescimosContasProgramaEspecial = valorTotalAcrescimosContasProgramaEspecial.add(contaValoresHelper.getValorAtualizacaoMonetaria()); 
						}
						
						if (contaValoresHelper.getValorJurosMora() != null) {
							valorTotalAcrescimosContasProgramaEspecial = valorTotalAcrescimosContasProgramaEspecial.add(contaValoresHelper.getValorJurosMora()); 
						}
						
						if (contaValoresHelper.getValorMulta() != null) {
							valorTotalAcrescimosContasProgramaEspecial = valorTotalAcrescimosContasProgramaEspecial.add(contaValoresHelper.getValorMulta()); 
						}
					}else{
		...
		retorno.put("valorTotalAcrescimosContasProgramaEspecial",valorTotalAcrescimosContasProgramaEspecial);
