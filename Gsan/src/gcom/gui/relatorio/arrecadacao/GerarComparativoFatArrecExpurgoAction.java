...
public class GerarComparativoFatArrecExpurgoAction extends ExibidorProcessamentoTarefaRelatorio {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		Collection<Integer> idsPerfilImovel = new ArrayList<Integer>();
		
		if (form.getIdsPerfilImovel() != null) {
			for (int i = 0; i < form.getIdsPerfilImovel().length; i++) {
				String idPerfilImovel = form.getIdsPerfilImovel()[i];
				
				if (idPerfilImovel != null && !idPerfilImovel.trim().equals("") && !idPerfilImovel.trim().equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)) {
					idsPerfilImovel.add(new Integer(idPerfilImovel));
				}
			}
		}
		
		if (opcaoTotalizacao.trim().equals("" + ConstantesSistema.CODIGO_GERENCIA_REGIONAL)) {
			gerenciaRegional = form.getIdGerenciaRegional();
			if (gerenciaRegional == null || gerenciaRegional.equals("") || gerenciaRegional.equals(ConstantesSistema.NUMERO_NAO_INFORMADO)) {
				throw new ActionServletException("atencao.required", null, "Gerência Regional");
			}
		}
		
		if (opcaoTotalizacao.trim().equals("" + ConstantesSistema.CODIGO_UNIDADE_NEGOCIO)) {
			unidadeNegocio = form.getIdUnidadeNegocio();
		...
		relatorio.addParametro("idsPerfilImovel", idsPerfilImovel);
