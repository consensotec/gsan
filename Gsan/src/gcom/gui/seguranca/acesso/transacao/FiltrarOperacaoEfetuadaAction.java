...
//			sessao.removeAttribute("horaFinal");
 ...
 if ( sessao.getAttribute("acao") == null || !"relatorio".equalsIgnoreCase(sessao.getAttribute("acao").toString())) {
			...
			if( form.getValorArgumentoPesquisa() != null && !form.getValorArgumentoPesquisa().equals("") ){
				...
				FiltroUsuario filtroUsur = new FiltroUsuario();
				
				if(form.getIdArgumentoPesquisa().equals(TabelaColuna.USUARIO_ID)){
					
					if(form.getValorArgumentoPesquisa().length() > ConstantesSistema.VALOR_QUATRO){
						filtroUsur.adicionarParametro(new ParametroSimples(FiltroUsuario.LOGIN, form.getValorArgumentoPesquisa()));
					}else{
						filtroUsur.adicionarParametro(new ParametroSimples(FiltroUsuario.ID, form.getValorArgumentoPesquisa()));
					}
					
					Collection colecaoUsuario = fachada.pesquisar(filtroUsur, Usuario.class.getName());
