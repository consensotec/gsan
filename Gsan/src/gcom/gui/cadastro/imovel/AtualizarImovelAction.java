		...
		if (sessao.getAttribute("POPUP") != null) {
			if (sessao.getAttribute("POPUP").equals("true")) {
				if (colecaoEnderecos != null && !colecaoEnderecos.isEmpty()) {
					Object obj = colecaoEnderecos.iterator().next();
...
/*		SistemaParametro sistemaParametro = fachada.pesquisarParametrosDoSistema();
		...
		//8.3 Caso tenha alterado o perfil e o perfil anterior ou o novo perfil tenha permiss�o especial
		//insere o hist�rico do enquadramento/desenquadramento na tabela CADASTRO.IMOVEL_HISTORICO_PERFIL
		ImovelPerfil imovelPerfilAnterior = new ImovelPerfil();
		FiltroImovel filtroImovel = new FiltroImovel();
		filtroImovel.adicionarParametro(new ParametroSimples(FiltroImovel.ID, inserirImovelHelper.getImovel().getId()));
		...
		if(!Util.isVazioOrNulo(colImovel)){
			Imovel imovelAnterior = (Imovel) colImovel.iterator().next();
			imovelPerfilAnterior = imovelAnterior.getImovelPerfil();
		}
		fachada.atualizarHistoricoEnquadramentoPerfil(inserirImovelHelper.getImovel(), inserirImovelHelper.getUsuario(), imovelPerfilAnterior, 
			PerfilAlteracaoMotivo.ALTERACAO_CADASTRO);
