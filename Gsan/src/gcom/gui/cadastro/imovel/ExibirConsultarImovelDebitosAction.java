...
import gcom.cobranca.HistoricoDebitosClienteImovelHelper;
...
public class ExibirConsultarImovelDebitosAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if(sessao.getAttribute("atualizacaoCadastral") != null){
			indicadorNovo = "1";
			sessao.removeAttribute("atualizacaoCadastral");
		}
		...
		if (limparForm != null && !limparForm.equals("")) {
			...
			sessao.removeAttribute("colecaoHistoricoDebitosClienteImovel");
			sessao.removeAttribute("totalDocumentosGeral");
			sessao.removeAttribute("totalValorDocumentosGeral");
...
//				}
				...
				if (imovelNovoPesquisado) {
					...
					//Soma das Guias e Contas do Imovel
					//Variavel usada na totalizacao do grid de Historico de Clientes com Debito
					Integer totalDocumentosGeral = new Integer(0);
					//Soma dos valores das Guias e Contas do Imovel
					//Variavel usada na totalizacao do grid de Historico de Clientes com Debito
					BigDecimal totalValorDocumentosGeral = new BigDecimal("0.00");
					
					HashMap<String, HistoricoDebitosClienteImovelHelper> historicoDebitosClienteImovelMap = gerarHistoricoDebitosClienteImovel(idImovelDebitos);
					HashMap<String, HistoricoDebitosClienteImovelHelper> clienteImovelComDebitoMap = new HashMap<String, HistoricoDebitosClienteImovelHelper>();
					...
					if (colecaoContaValores != null	&& !colecaoContaValores.isEmpty()) {
						...
						while (colecaoContaValoresIterator.hasNext()) {
							...
							/*******************************************************************************
							 * Insere no maps do grid "Historico Clientes com Debido" as contas por cliente
							 * *****************************************************************************/
							...
							filtroClienteConta.adicionarParametro(new ParametroSimples(FiltroClienteConta.CONTA_ID, dadosConta.getConta().getId()));
							filtroClienteConta.adicionarParametro(new ParametroSimples(FiltroClienteConta.INDICADOR_NOME_CONTA, ConstantesSistema.SIM));
							Collection colecaoClienteConta = fachada.pesquisar(filtroClienteConta, ClienteConta.class.getName());
							ClienteConta clienteConta = (ClienteConta) Util.retonarObjetoDeColecao(colecaoClienteConta);
							
							HistoricoDebitosClienteImovelHelper historicoDebitosClienteImovelHelper = new HistoricoDebitosClienteImovelHelper();
							
							if(clienteConta != null){
								historicoDebitosClienteImovelHelper	= historicoDebitosClienteImovelMap.get(clienteConta.getCliente().getId() + "|" + clienteConta.getClienteRelacaoTipo().getId());
							}
							if(historicoDebitosClienteImovelHelper != null){
								historicoDebitosClienteImovelHelper.getContasClienteImovelHelper().add(dadosConta);	
							
								historicoDebitosClienteImovelHelper.setValorTotalConta(historicoDebitosClienteImovelHelper.getValorTotalConta().add(dadosConta.getConta().getValorTotal()));
								historicoDebitosClienteImovelHelper.setValorTotalAcrescimo(historicoDebitosClienteImovelHelper.getValorTotalAcrescimo().add(dadosConta.getValorTotalContaValores()));							
								historicoDebitosClienteImovelHelper.setValorTotalAgua(historicoDebitosClienteImovelHelper.getValorTotalAgua().add(dadosConta.getConta().getValorAgua()));
								historicoDebitosClienteImovelHelper.setValorTotalEsgoto(historicoDebitosClienteImovelHelper.getValorTotalEsgoto().add(dadosConta.getConta().getValorEsgoto()));
								historicoDebitosClienteImovelHelper.setValorTotalDebito(historicoDebitosClienteImovelHelper.getValorTotalDebito().add(dadosConta.getConta().getDebitos()));
								historicoDebitosClienteImovelHelper.setValorTotalCredito(historicoDebitosClienteImovelHelper.getValorTotalCredito().add(dadosConta.getConta().getValorCreditos()));
								historicoDebitosClienteImovelHelper.setValorTotalImposto(historicoDebitosClienteImovelHelper.getValorTotalImposto().add(dadosConta.getConta().getValorImposto()));
								historicoDebitosClienteImovelHelper.setQuantidadeContas(historicoDebitosClienteImovelHelper.getQuantidadeContas() + 1);
								
								historicoDebitosClienteImovelHelper.setTotalDocumento(historicoDebitosClienteImovelHelper.getTotalDocumento() + 1);
								historicoDebitosClienteImovelHelper.setValorTotalDebitos(historicoDebitosClienteImovelHelper.getValorTotalDebitos().add(dadosConta.getConta().getValorTotal()));
							
								clienteImovelComDebitoMap.put(clienteConta.getCliente().getId() + "|" + clienteConta.getClienteRelacaoTipo().getId(), historicoDebitosClienteImovelHelper);
								
								totalDocumentosGeral = totalDocumentosGeral + 1;
								totalValorDocumentosGeral = totalValorDocumentosGeral.add(dadosConta.getConta().getValorTotal());
							}
						}
					}
					...
					if (colecaoGuiaPagamentoValores != null	&& !colecaoGuiaPagamentoValores.isEmpty()) {
						...
						while (colecaoGuiaPagamentoValoresHelperIterator.hasNext()) {
							...
							/*******************************************************************************
							 * Insere no maps do grid "Historico Clientes com Debido" as Guias de Pagamento por cliente
							 * *****************************************************************************/
							
							FiltroClienteGuiaPagamento filtroClienteGuia = new FiltroClienteGuiaPagamento();
							filtroClienteGuia.adicionarParametro(new ParametroSimples(FiltroClienteGuiaPagamento.GUIA_PAGAMENTO_ID, dadosGuiaPagamentoValoresHelper.getGuiaPagamento().getId()));
							Collection colecaoClienteGuia = fachada.pesquisar(filtroClienteGuia, ClienteGuiaPagamento.class.getName());
							ClienteGuiaPagamento clienteGuia = (ClienteGuiaPagamento) Util.retonarObjetoDeColecao(colecaoClienteGuia);
							
							HistoricoDebitosClienteImovelHelper historicoDebitosClienteImovelHelper = new HistoricoDebitosClienteImovelHelper();
							if(clienteGuia != null){
								historicoDebitosClienteImovelHelper	= historicoDebitosClienteImovelMap.get(clienteGuia.getCliente().getId() + "|" + clienteGuia.getClienteRelacaoTipo().getId());
								if(historicoDebitosClienteImovelHelper != null){
									historicoDebitosClienteImovelHelper.getGuiasPagamentoClienteImovel().add(dadosGuiaPagamentoValoresHelper);
									historicoDebitosClienteImovelHelper.setValorTotalGuia(historicoDebitosClienteImovelHelper.getValorTotalGuia().add(dadosGuiaPagamentoValoresHelper.getGuiaPagamento().getValorDebito()));
									historicoDebitosClienteImovelHelper.setQuantidadeGuias(historicoDebitosClienteImovelHelper.getQuantidadeGuias() + 1);
									
									historicoDebitosClienteImovelHelper.setTotalDocumento(historicoDebitosClienteImovelHelper.getTotalDocumento() + 1);
									historicoDebitosClienteImovelHelper.setValorTotalDebitos(historicoDebitosClienteImovelHelper.getValorTotalDebitos().add(dadosGuiaPagamentoValoresHelper.getGuiaPagamento().getValorDebito()));
									
									clienteImovelComDebitoMap.put(clienteGuia.getCliente().getId() + "|" + clienteGuia.getClienteRelacaoTipo().getId(), historicoDebitosClienteImovelHelper);
									
									totalDocumentosGeral = totalDocumentosGeral + 1;
									totalValorDocumentosGeral = totalValorDocumentosGeral.add(dadosGuiaPagamentoValoresHelper.getGuiaPagamento().getValorDebito());
								}
							}
						}
					}
					...
					if ((colecaoContaValores == null)&& (colecaoDebitoACobrar == null || colecaoDebitoACobrar.isEmpty())
						...
						sessao.setAttribute("colecaoHistoricoDebitosClienteImovel", clienteImovelComDebitoMap.values());
						sessao.setAttribute("totalDocumentosGeral", totalDocumentosGeral);
						sessao.setAttribute("totalValorDocumentosGeral", totalValorDocumentosGeral);
						...
						if(sistemaParametro.getResolucaoDiretoria() != null){
							...
							if (valorDebitoACobrarSemJurosParcelamento != null && valorDebitoACobrarSemJurosParcelamento.compareTo(BigDecimal.ZERO) > 0) {
								valorTotalDescontoPagamentoAVista = valorTotalDescontoPagamentoAVista.add(valorDebitoACobrar).subtract(valorDebitoACobrarSemJurosParcelamento);
							}
							
							BigDecimal valorPagamentoAVista = valorTotalComAcrescimo.subtract(valorTotalDescontoPagamentoAVista);

							sessao.setAttribute("obterOpcoesDeParcelamentoHelper", helper);
				...
				sessao.removeAttribute("historicoDebitosClienteImovelMap");
				sessao.removeAttribute("colecaoHistoricoDebitosClienteImovel");
				sessao.removeAttribute("totalDocumentosGeral");
				sessao.removeAttribute("totalValorDocumentosGeral");
	...
	/***
	 * Cria um map com todos os clientesImovel do imovel informado.
	 * 
	 * [UC0203] - Consultar Debitos
	 * 21. Histórico de clientes com débitos
	 * 
	 * @author Anderson Cabral
	 ******/	
	private HashMap<String, HistoricoDebitosClienteImovelHelper> gerarHistoricoDebitosClienteImovel(String idImovelDebitos){
		
			//Pesquisar todos os clientes do Imovel
			Collection<ClienteImovel> historicoClientesImovel = this.getFachada().pesquisarClientesImovelDadosComplementares(new Integer(idImovelDebitos.trim()));
			HashMap<String, HistoricoDebitosClienteImovelHelper> historicoClientesImovelMap = new HashMap<String, HistoricoDebitosClienteImovelHelper>();
				
			for(ClienteImovel clienteImovel : historicoClientesImovel){
				Integer idCliente = clienteImovel.getCliente().getId();
				
				HistoricoDebitosClienteImovelHelper historicoDebitosClienteImovelHelper 
												= new HistoricoDebitosClienteImovelHelper();
				historicoDebitosClienteImovelHelper.setClienteImovel(clienteImovel);
				
				historicoClientesImovelMap.put(idCliente.toString() +"|"+ clienteImovel.getClienteRelacaoTipo().getId(), historicoDebitosClienteImovelHelper);
		}		
			return historicoClientesImovelMap;
	}
