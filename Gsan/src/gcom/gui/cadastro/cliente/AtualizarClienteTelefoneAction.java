 ...
 * @author Rodrigo
 */
public class AtualizarClienteTelefoneAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if ( colecaoFones != null && !colecaoFones.isEmpty()) {
			...
			Long indicadorTelefoneSMS = (Long) clienteActionForm.get("indicadorTelefoneSMS");

			// Se o telefone padrão for escolhido então o objeto deve ser alterado
			if (indicadorTelefonePadrao != null) {
				Iterator iterator = colecaoFones.iterator();

				// Varre a colecão para descobrir o objeto que tem o telefone principal
				while (iterator.hasNext()) {
					ClienteFone clienteFone = (ClienteFone) iterator.next();

					if (obterTimestampIdObjeto(clienteFone) == indicadorTelefonePadrao.longValue()) {
					...
					if (indicadorTelefoneSMS != null && obterTimestampIdObjeto(clienteFone) == indicadorTelefoneSMS
							.longValue()) {
						
						//[FS0019] Verificar tipo de telefone
						//Caso o Indicador de "Telefone SMS" esteja marcado com "SIM" e o Tipo do Telefone não for "3 - Celular"
						if( clienteFone.getFoneTipo().getId() != FoneTipo.CELULAR.intValue()){
							throw new ActionServletException("atencao.telefone_sms_tipo_cel");
						}
						
						
						clienteFone
								.setIndicadorTelefoneSMS(ConstantesSistema.SIM);
					} else {
						
						clienteFone
								.setIndicadorTelefoneSMS(null);
					}
				}

			} else {
				// Nenhum telefone foi indicado como principal Mostra o erro
