 ...
 * @date 18/06/2015
 */
public class InserirCadastroContaBraillePortalCaerAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if (httpServletRequest.getParameter("confirmar") == null
			...
			if (form.getEmail() != null && !form.getEmail().equals("")) {	
				...
				if (!email.matches("[a-zA-Z0-9]+[a-zA-Z0-9_.-]+@{1}[a-zA-Z0-9_.-]*\\.+[a-z]{2,4}")) {
			...
			if(form.getTelefoneContato() != null && form.getTelefoneContato().length() >= 13){
				String telefone = form.getTelefoneContato().trim();
				telefone = telefone.replace("(", "");
				telefone = telefone.replace(")", "");
				telefone = telefone.replace("-", "");
				telefone = telefone.replace(" ", "");
				if(telefone.trim().length() == 10 || telefone.trim().length() == 11 ){
