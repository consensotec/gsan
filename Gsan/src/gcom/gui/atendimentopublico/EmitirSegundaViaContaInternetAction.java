...
//			mes = dataCorrenteTexto.substring(3, 5);
			...
			String valorParametro = this.getFachada().obterValorParametro(ParametroSistema.INDICADOR_PERGUNTA_2_VIA_CLIENTE_IMOVEL);
			
			if (valorParametro != null && valorParametro.equals(ConstantesSistema.SIM.toString())) {
				this.verificarClientesConta(form, colecaoContasValores);
			}
	...
	public void verificarClientesConta(EmitirSegundaViaContaInternetActionForm form, Collection<?> colecaoContas){
		FiltroClienteImovel filtroClienteImovel = new FiltroClienteImovel();
		filtroClienteImovel.adicionarParametro(new ParametroSimples(
				FiltroClienteImovel.IMOVEL_ID, form.getMatricula()));
		filtroClienteImovel.adicionarParametro(new ParametroNulo(FiltroClienteImovel.DATA_FIM_RELACAO));
		filtroClienteImovel.adicionarParametro(new ParametroSimples(
				FiltroClienteImovel.INDICADOR_NOME_CONTA, ConstantesSistema.SIM));
		filtroClienteImovel.adicionarCaminhoParaCarregamentoEntidade(FiltroClienteImovel.CLIENTE);
		
		Collection<?> colClienteImovel = this.getFachada().pesquisar(filtroClienteImovel, ClienteImovel.class.getName());
		...
		if(!Util.isVazioOrNulo(colClienteImovel)){
			if(!Util.isVazioOrNulo(colecaoContas)){
				...
				while(iterator.hasNext()){
					helper = (ContaValoresHelper) iterator.next();
					
					filtroClienteConta = new FiltroClienteConta();
					filtroClienteConta.adicionarParametro(new ParametroSimples(FiltroClienteConta.CONTA_ID, helper.getConta().getId()));
					...
					clienteConta = (ClienteConta) Util.retonarObjetoDeColecao(this.getFachada().pesquisar(filtroClienteConta, ClienteConta.class.getName()));
					
					if(clienteConta != null && clienteConta.getCliente() != null && clienteImovel.getCliente() != null){
						if(!String.valueOf(clienteImovel.getCliente().getId()).equals(String.valueOf(clienteConta.getCliente().getId()))){
							helper.setIndicadorMensagem(ConstantesSistema.SIM);
							helper.setIdClienteConta(clienteConta.getCliente().getId());
							helper.setIdClienteImovel(clienteImovel.getCliente().getId());
						}else{
							helper.setIndicadorMensagem(ConstantesSistema.NAO);
							helper.setIdClienteConta(clienteConta.getCliente().getId());
							helper.setIdClienteImovel(clienteImovel.getCliente().getId());
						}
					}
