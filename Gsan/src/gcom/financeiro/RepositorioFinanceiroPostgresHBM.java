...
//	*******************************Caern	
	...
	update = 	"UPDATE faturamento.conta as cnta "
			...
			st.setInt(6, DebitoCreditoSituacao.INCLUIDA);
			st.setInt(7, DebitoCreditoSituacao.RETIFICADA);
			st.setInt(8, anoMesReferenciaContabil);				
			st.setInt(9, idParametrosDevedoresDuvidosos );
			st.setInt(10, anoMesReferenciaContabil );
			st.setInt(11, anoMesReferenciaContabil );
			st.setInt(12, anoMesReferenciaContabil );
			st.setInt(13, anoMesReferenciaContabil );
	...
	public Integer pesquisaQuantidadeContasBaixaPerdasOrgaoPublico(Integer quantidadeMinimaInformada, Collection<Integer> colecaoEsferaPoder ) 
		...
		consulta =  "select count(*) as quantidade from faturamento.conta conta "
			    ...
			    +   "and conta.cnta_id not in ( select pag.cnta_id from arrecadacao.pagamento pag ) "
				+   "and abs((( extract( year from to_date( conta.cnta_dtvencimentoconta , 'YYYY-MM-DD' ) ) - extract( year from to_date( :dataAtual, 'YYYY-MM-DD' ) ) ) * 12 ) + "
				+	"( extract( month from to_date(  conta.cnta_dtvencimentoconta, 'YYYY-MM-DD' )  ) ) - extract( month from to_date( :dataAtual, 'YYYY-MM-DD' ) ) ) >= :quantidadeMinimaInformada "
		...
		try {
			Connection jdbcCon = session.connection();
			  ...
			  update =  "update faturamento.conta cnta "
					...
					+   "where cnta.loca_id = ? "
					+   "and cnta.qdra_id = ? "
					+   "and cnta.cnta_id in (select conta.cnta_id from faturamento.conta conta "  
											+   "inner join cadastro.cliente_conta clco on clco.cnta_id = conta.cnta_id "
											+   "inner join cadastro.cliente clie on clie.clie_id = clco.clie_id "
											+   "inner join cadastro.cliente_tipo cltp on cltp.cltp_id = clie.cltp_id "
											+   "inner join faturamento.conta_categoria catg on catg.cnta_id = conta.cnta_id "
										    ...
										    +   "and conta.cnta_amreferenciabaixacontabil is null " 
									 	    +   "and conta.cnta_amreferenciabaixasocial is null "
										    +   "and catg.catg_id = ? "
										    +   "and conta.cnta_id not in ( select pag.cnta_id from arrecadacao.pagamento pag where pag.cnta_id = conta.cnta_id) "
										    +   "and abs((( extract( year from to_date( conta.cnta_dtvencimentoconta , 'YYYY-MM-DD' ) ) - extract( year from to_date( now(), 'YYYY-MM-DD' ) ) ) * 12 ) + "
											+	"( extract( month from to_date(  conta.cnta_dtvencimentoconta, 'YYYY-MM-DD' )  ) ) - extract( month from to_date( now(), 'YYYY-MM-DD' ) ) ) >= ? "
