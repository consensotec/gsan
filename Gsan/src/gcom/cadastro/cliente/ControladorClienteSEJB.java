...
//		registradorOperacao.registrarOperacao(clienteFone);
				...
				while(iteratorConta.hasNext()){
					...
					if(dataLeitura != null){
						...
						if( dadosClienteImovel != null && !dadosClienteImovel.isEmpty() ){
							...
							boolean colocarContaRevisao = this.verificarColocarContaRevisao(colClienteImovelHelper, dataLeitura);
							
							if (colocarContaRevisao) {
								// Coloca a conta em revisão
								ContaMotivoRevisao contaMotivoRevisao = new ContaMotivoRevisao();
								contaMotivoRevisao.setId(ContaMotivoRevisao.MUDANCA_CLIENTE_RESPONSAVEL);
								...
								repositorioFaturamento.colocarContaRevisao(conta);
							}
	 ...
	 * [UC 1366] - Alterar Vínculo de Clientes com o Imóvel e Contas
	 * 
	 * Método responsavel por verificar se existe cliente imovel.
	 * 
	 * @author Rafael Corrêa
	 * @date 24/04/2015
	 */
	 ...
	 * [UC 1366] - Alterar Vínculo de Clientes com o Imóvel e Contas
	 * 
	 * Método responsavel por verificar se existe cliente conta.
	 * 
	 * @author Rafael Corrêa
	 * @date 24/04/2015
	 */
		...
		if(tipo.equals("2")){
			...
			while(it.hasNext()){
				...
				if(clienteImovelHelper.getDataFimRelacao() != null){
					...
					if(clienteImovelHelper.getIdClienteImovelFimRelacaoMotivo() != null && !clienteImovelHelper.getIdClienteImovelFimRelacaoMotivo().trim().equals(""+ConstantesSistema.NUMERO_NAO_INFORMADO)){
	 ...
	 * [UC 1366] - Alterar Vínculo de Clientes com o Imóvel e Contas
	 * 
	 * Verifica se o usuário marcou o cliente associado a conta para colocar em revisão
	 * 
	 * @author Rafael Corrêa
	 * @date 27/04/2015
	 */
	private boolean verificarColocarContaRevisao(Collection colecaoClienteImovelHelper, Date dataLeitura) {
		boolean retorno = false;

		if (colecaoClienteImovelHelper != null && !colecaoClienteImovelHelper.isEmpty()) {
			Iterator<?> it = colecaoClienteImovelHelper.iterator();
			while(it.hasNext()){
				ClienteImovelHelper clienteImovelHelper = (ClienteImovelHelper) it.next();
				
				if (clienteImovelHelper.getIndicadorRevisao() != null && clienteImovelHelper.getIndicadorRevisao().equals(ConstantesSistema.SIM.toString())) {
					Date dataInicioRelacao = Util.converteStringParaDate(clienteImovelHelper.getDataInicioRelacao());
					Date dataFimRelacao = Util.converteStringParaDate(clienteImovelHelper.getDataFimRelacao());
					
					if (dataLeitura.compareTo(dataInicioRelacao) >= 0 && (dataFimRelacao == null || dataLeitura.compareTo(dataFimRelacao) <= 0)) {
						retorno = true;
					}
				}
			}
