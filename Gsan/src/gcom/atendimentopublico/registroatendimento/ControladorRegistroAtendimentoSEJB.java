...
import gcom.gui.portal.caema.ConsultarTramiteHelper;
...
//			getControladorFaturamento().inserirCreditoRealizadoCategoria(creditoRealizadoInserir,colecaoCategoria);
	...
	public void validarOcorrenciaOperacional(OcorrenciaOperacional ocorrenciaOperacional, boolean acao)
		...
		filtroOcorrenciaOperacional.adicionarParametro(new ParametroSimples(FiltroOcorrenciaOperacional.OCORRENCIA_OPERACIONAL_MOTIVO_ID, 
																			ocorrenciaOperacional.getOcorrenciaOperacionalMotivo().getId()));
		...
		if (colecaoOcorrenciaOperacional != null && !colecaoOcorrenciaOperacional.isEmpty()) {
			...
			if (acao) {
				...
				throw new ControladorException(	"atencao.ocorrencia_operacional_ja_existente",
												...
												ocorrenciaOperacionalJaCadastrado.getOcorrenciaOperacionalMotivo().getDescricao());
					...
					&& ocorrenciaOperacional.getOcorrenciaOperacionalMotivo().getId().toString().equals(ocorrenciaOperacionalJaCadastrado.getOcorrenciaOperacionalMotivo().getId().toString())) {
	 ...
	 * @date 24/04/2015
	 */
	public Integer obterRAVinculo(Integer idImovel) throws ControladorException {
		try{
			return repositorioRegistroAtendimento.obterRAVinculo(idImovel);
		}catch (ErroRepositorioException ex) {
	        throw new ControladorException("erro.sistema", ex);
	    }
	}
