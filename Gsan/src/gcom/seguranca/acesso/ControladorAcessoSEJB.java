...
//						"atencao.data_nascimento.incorreta.login", null, login);
	...
	public String verificarTipoURL(String url) throws ControladorException {
		...
		if (url.contains("?")) {
			parametro = url.substring(url.indexOf("?") + 1, url.length());
		...
		if (colecaoFuncionalidade != null && !colecaoFuncionalidade.isEmpty()) {
			...
			colecaoFuncionalidade = getControladorUtil().pesquisar(
					filtroFuncionalidade, Funcionalidade.class.getName());
	...
	public boolean verificarAcessoPermitidoFuncionalidade(
		...
		if(idFuncionalidade != null){
			...
			if (urlFuncionalidade.contains("?")) {
				parametro = urlFuncionalidade.substring(urlFuncionalidade.indexOf("?") + 1, urlFuncionalidade.length());
				urlFuncionalidade = urlFuncionalidade.substring(0, urlFuncionalidade.indexOf("?"));
			}
